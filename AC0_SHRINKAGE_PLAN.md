...
**Ответ:**
- **Нет**, `M²` не годится как финальная оценка для цели P≠NP.
...
- Значит, финальные теоремы либо становятся слабее, либо не применимы к нужным
  диапазонам параметров.
...
И downstream обязан использовать только `ac0DepthBound`.
