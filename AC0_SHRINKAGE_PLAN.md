...
* В `pnp3/` остаётся **одна активная внешняя аксиома**:
  `ThirdPartyFacts.ppoly_circuit_locality` (NP‑hardness из P/poly locality bridge).
...
## C2. Вся цепочка P≠NP с одной внешней аксиомой

После закрытия C1 остаётся только **одна** внешняя зависимость:
`ppoly_circuit_locality`.

**DoD C2**:
* `#print axioms P_ne_NP_final` (или финальный модуль) показывает
  только `ppoly_circuit_locality` + базовые классические аксиомы.
