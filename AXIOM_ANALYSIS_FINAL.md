...
но требуют внешних свидетелей (`AC0CircuitWitness` и `LocalCircuitWitness`)
через `FamilyIsAC0`/`FamilyIsLocalCircuit`.
...
переведены в статус теорем; обе требуют внешних witness-объектов.

---

## 6. Выводы

1. В актуальной ветке **нет внешних аксиом**; ключевые shrinkage-леммы требуют
   внешних witness-объектов.
...
4. **Дальнейшая работа** должна сосредоточиться на предоставлении shrinkage-
   свидетелей для `partial_shrinkage_for_AC0` и `shrinkage_for_localCircuit`.
