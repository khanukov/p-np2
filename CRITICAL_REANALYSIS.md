...
(D.2, теперь доказанный). Остальные семь аксиом поддерживают обобщения
...
| Part D | `OPS_trigger_general`, `Locality_trigger`, `CJW_sparse_trigger` | Magnification |
...
- `pnp3/Magnification/Facts_Magnification.lean` — три триггера остаются аксиомами
  (`OPS_trigger_general`, `Locality_trigger`, `CJW_sparse_trigger`), формульный
  триггер доказан как специализация.
...
3. **Magnification (Part D)** — наилучший кандидат для ближайших проектов; цель —
   закрыть оставшиеся триггеры (`OPS_trigger_general`, `Locality_trigger`,
   `CJW_sparse_trigger`).
