...
**Текущий прогресс (2025-10-24 обновление)**:

- ✅ **D.1**: Аксиома `OPS_trigger_general_contra` заменена на доказательство.
  Контрапозиция теперь выводится из `GeneralCircuitStatement`, полученного
  на шаге C (Locality-Lift + античекер), и конструктивного извлечения
  решателя из предположения `NP ⊆ P/poly`. Осталось только дальнейшее
