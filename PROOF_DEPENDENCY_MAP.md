...
3. ⚠️ **Оставить A.1, A.2, C.7 и D.2/D.3 как axioms с ДЕТАЛЬНОЙ ДОКУМЕНТАЦИЕЙ**

**Результат**:
- Формальное доказательство P≠NP **модуло 5 внешних фактов**
...
- **8 активных аксиом** (switching + anti-checker + локальные/разреженные триггеры)
- Базовый путь к P≠NP полностью формален, триггер `OPS_trigger_general` закрыт доказательством
...
**Задача**: Заменить аксиомы `Locality_trigger` и `CJW_sparse_trigger` на доказательства
  (формульная и общая версии уже доказаны через `OPS_trigger_general`).
...
**Возможный результат**: 8 → 6 аксиом
