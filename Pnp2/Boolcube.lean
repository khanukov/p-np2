...
/--
The *enumeration complexity* of the left block.  When exploring the points of a
subcube by brute force, one may freely assign values to every unfixed left
coordinate.  The search therefore branches over `2^(#freeLeft)` possibilities.
-/
def leftEnumerationBound (R : Subcube n) (k : ℕ) : ℕ :=
  Nat.pow 2 (freeLeft (n := n) R k).card

/--
The analogous enumeration bound for the right block of coordinates.
-/
def rightEnumerationBound (R : Subcube n) (k : ℕ) : ℕ :=
  Nat.pow 2 (freeRight (n := n) R k).card

@[simp] lemma leftEnumerationBound_def (R : Subcube n) (k : ℕ) :
    leftEnumerationBound (n := n) R k
      = Nat.pow 2 (freeLeft (n := n) R k).card := rfl

@[simp] lemma rightEnumerationBound_def (R : Subcube n) (k : ℕ) :
    rightEnumerationBound (n := n) R k
      = Nat.pow 2 (freeRight (n := n) R k).card := rfl

/--
Bounding the number of free left coordinates directly bounds the enumeration
complexity: the power function is monotone in the exponent for bases ≥ 2.
-/
lemma leftEnumerationBound_le_of_card_le {R : Subcube n} {k r : ℕ}
    (h : (freeLeft (n := n) R k).card ≤ r) :
    leftEnumerationBound (n := n) R k ≤ Nat.pow 2 r := by
  classical
  simpa [leftEnumerationBound_def]
    using Nat.pow_le_pow_right (by decide : 1 ≤ (2 : ℕ)) h

/--
Right-hand version of `leftEnumerationBound_le_of_card_le`.
-/
lemma rightEnumerationBound_le_of_card_le {R : Subcube n} {k r : ℕ}
    (h : (freeRight (n := n) R k).card ≤ r) :
    rightEnumerationBound (n := n) R k ≤ Nat.pow 2 r := by
  classical
  simpa [rightEnumerationBound_def]
    using Nat.pow_le_pow_right (by decide : 1 ≤ (2 : ℕ)) h

/--
Translate the cardinal identity `#freeLeft = #left − #fixedLeft` into an
explicit inequality on enumeration bounds.  Provided that the left block indeed
consists of the first `k` coordinates (`k ≤ n`), fixing at least `t` of them
reduces the brute-force search to `2^{k - t}` branches.
-/
lemma leftEnumerationBound_le_pow_sub {R : Subcube n} {k t : ℕ}
    (hk : k ≤ n) (hfix : t ≤ (fixedLeft (n := n) R k).card) :
    leftEnumerationBound (n := n) R k ≤ Nat.pow 2 (k - t) := by
  classical
  -- Express the number of free left coordinates via the complementary count of
  -- fixed ones.
  have hfree := card_freeLeft_eq_left_sub_fixed (n := n) (R := R) (k := k)
  -- The left block has exactly `k` elements under the assumption `k ≤ n`.
  have hleft_card : (Coords.left n k).card = k := by
    simpa [Coords.card_left, Nat.min_eq_left hk] using (Coords.card_left n k)
  -- Directly bound `#freeLeft` using the previous identity.
  have hfree_le : (freeLeft (n := n) R k).card ≤ k - t := by
    have hleft_le : (Coords.left n k).card ≤ k := by
      simpa [hleft_card]
    have hsub_le :
        (Coords.left n k).card - (fixedLeft (n := n) R k).card
          ≤ (Coords.left n k).card - t :=
        Nat.sub_le_sub_left hfix _
    have hsub_left :
        (Coords.left n k).card - t ≤ k - t :=
        Nat.sub_le_sub_right hleft_le _
    simpa [hfree, hleft_card] using hsub_le.trans hsub_left
  -- Transfer the bound on the exponent to the enumeration cost.
  exact leftEnumerationBound_le_of_card_le (n := n) (R := R) (k := k)
    (r := k - t) hfree_le

/--
Mirror image of `leftEnumerationBound_le_pow_sub` for the right block.  When
`k ≤ n`, the right block contains exactly `n - k` coordinates.
-/
lemma rightEnumerationBound_le_pow_sub {R : Subcube n} {k t : ℕ}
    (hk : k ≤ n) (hfix : t ≤ (fixedRight (n := n) R k).card) :
    rightEnumerationBound (n := n) R k ≤ Nat.pow 2 (n - k - t) := by
  classical
  have hfree := card_freeRight_eq_right_sub_fixed (n := n) (R := R) (k := k)
  -- `Coords.right` hosts `n - k` indices under the assumption `k ≤ n`.
  have hright_card : (Coords.right n k).card = n - k := by
    simpa [Coords.card_right, Nat.min_eq_left hk] using (Coords.card_right n k)
  have hfree_le : (freeRight (n := n) R k).card ≤ n - k - t := by
    have hright_le : (Coords.right n k).card ≤ n - k := by
      simpa [hright_card]
    have hsub_le :
        (Coords.right n k).card - (fixedRight (n := n) R k).card
          ≤ (Coords.right n k).card - t :=
        Nat.sub_le_sub_left hfix _
    have hsub_right :
        (Coords.right n k).card - t ≤ n - k - t :=
        Nat.sub_le_sub_right hright_le _
    simpa [hfree, hright_card] using hsub_le.trans hsub_right
  exact rightEnumerationBound_le_of_card_le (n := n) (R := R) (k := k)
    (r := n - k - t) hfree_le
