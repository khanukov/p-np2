...
lemma powFamily_cover_for_member {n c : ℕ} (hn : 0 < n)
  ...
  refine ⟨Rset, ?_, ?_, ?_, ?_⟩
  · -- Rectangles survive the filter only if they already belonged to `Rset₀`.
    intro R hR g hg
...
lemma powFamily_cover_for_member_doubleExp {n c : ℕ}
  ...
  classical
  obtain ⟨Rset, hmonoFam, hmono, hcover, hcard⟩ :=
    powFamily_cover_for_member (n := n) (c := c) hn hf
  have hexp := Numeric.exponent_le_doubleExp (n := n) (c := c) hlarge
  have hpow := Nat.pow_le_pow_right (by decide : 1 ≤ (2 : ℕ)) hexp
  exact ⟨Rset, hmonoFam, hmono, hcover, hcard.trans hpow⟩
