...
lemma coloredSubcubesAux_cons_subset_node_perm (t₀ t₁ : DecisionTree n)
  ...
  obtain ⟨br₂, hbr₂, hsub₂⟩ :
      ∃ brRec ∈ coloredSubcubesAux (n := n) (DecisionTree.node j t₀ t₁) (p₁ ++ p₂),
        ∀ ⦃x : Point n⦄, Subcube.mem br₁.2 x → Subcube.mem brRec.2 x := by
    -- The proof of this step mirrors `coloredSubcubesAux_cons_subset` and will
    -- eventually be discharged by invoking that lemma once available.  For the
    -- moment we leave it as a placeholder.
    exact sorry
