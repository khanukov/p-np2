...
/-- `matches x p` means that the point `x` satisfies all assignments in `p`. -/
def matches (x : Point n) (p : List (Fin n × Bool)) : Prop :=
  ∀ q ∈ p, x q.1 = q.2

@[simp] lemma matches_nil (x : Point n) : matches (n := n) x [] := by
  intro q hq; cases hq

lemma matches_cons {x : Point n} {i : Fin n} {b : Bool} {p : List (Fin n × Bool)} :
    matches (n := n) x ((i, b) :: p) ↔ x i = b ∧ matches (n := n) x p := by
  constructor
  · intro h; refine ⟨h _ (by simp), ?_⟩
    intro q hq; exact h q (by simpa [hq] using List.mem_cons_of_mem _ hq)
...
lemma matches_toList_of_mem {R : Subcube n} {x : Point n}
    (hx : x ∈ₛ R) : matches (n := n) x (Subcube.toList (n := n) R) := by
  classical
  intro q hq
...
lemma eval_matchSubcube_matches {p : List (Fin n × Bool)} {b : Bool}
  ...
  | cons hd tl ih =>
      rcases hd with ⟨i, b'⟩
      have hcons := (matches_cons (x:=x) (i:=i) (b:=b') (p:=tl)) |>.mp h
      cases b' with
      | false =>
          have hx : x i = false := hcons.1
          simp [matchSubcube, hx, ih hcons.2]
      | true =>
...
lemma eval_ofRectCoverList_cons_mem {default : Bool} {b : Bool}
  ...
  simpa using
    (eval_matchSubcube_matches (n := n) (p := Subcube.toList (n := n) R)
      (b := b) (t := ofRectCoverList (n := n) default rs)
      (x := x) (matches_toList_of_mem (n := n) (R := R) hx))
