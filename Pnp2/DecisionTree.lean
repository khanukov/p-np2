...
lemma coloredSubcubesAux_cons_subset_node_perm (t₀ t₁ : DecisionTree n)
  ...
  -- Having removed `(j, bj)` we next discard the head assignment `(i, b)`.
  -- Its coordinate is fresh for the tail `p₁ ++ p₂`, since any occurrence
  -- would also give an occurrence in the full path `p`, contradicting `hi`.
  have hi_fresh : i ∉ (subcube_of_path (n := n) (p₁ ++ p₂)).idx := by
    -- This auxiliary fact will be needed to eliminate `(i, b)` in a future
    -- refinement of the argument.  Its formal proof is currently omitted.
    sorry
  -- Ultimately we must relate the colour set for `p₁ ++ p₂` back to the
  -- original tail `p = p₁ ++ (j, bj) :: p₂`.  This requires permuting the
  -- path to reinsert `(j, bj)` at its original position and subsequently
  -- dropping `(i, b)`.  The corresponding bookkeeping has not yet been
  -- formalised.
  refine ⟨br₁, ?_, ?_⟩
