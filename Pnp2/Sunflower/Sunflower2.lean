...
lemma sunflower_exists_easy
  ...
  -- Use `Finset.inf'` (infimum over a finite set) instead of the
  -- deprecated/non-existent `Finset.interFinset` helper.
  have hTnonempty : T.Nonempty := by
    -- Since `T.card = p` and `p ≥ 2`, the family `T` is nonempty.
    have : 0 < T.card := by
      simpa [hcardT] using (Nat.zero_lt_of_lt $ Nat.succ_le_of_lt hp)
    simpa [Finset.card_eq_zero] using this
  let core : Finset α := T.inf' hTnonempty id
  -- We package the intersection `core` into a sunflower structure.  The
  -- remaining proof follows the classical argument but is omitted here.
  have hSun : IsSunflower (α := α) p T core := by
    -- TODO: reimplement the detailed proof using `inf'`.
    refine ⟨by simpa [hcardT], ?_⟩
    sorry
  exact ⟨T, hsub, core, hSun⟩
...
-- Placeholder section reserved for future use by the cover construction.
