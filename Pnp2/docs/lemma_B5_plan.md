...
- [x] Просмотреть существующие формулировки в `Pnp2/bound.lean`, `Pnp2/Circuit/Numeric.lean`, `Pnp2/Cover/Bounds.lean`, `Pnp2/Cover/Sunflower.lean`, `Pnp2/cover2.lean` и связанных файлах, чтобы определить, где лучше разместить итоговую формулировку Леммы B-5 и какие модули уже импортируют необходимые определения (например, `Subcube`, `CoverFamily`, `canonicalCircuit`).
...
- [x] В модуле `Pnp2/Circuit/EntropyCover.lean` сформулирована теорема `smallCircuit_doubleExp_cover` (в комментарии отмечено соответствие Лемме B‑5), перечисляющая четыре свойства покрытия: монохроматичность, покрытие всех 1-точек функции, оценку числа прямоугольников $2^{2^n - 2^{n/2}}$ (с явным выбором $\delta = 1/2$) и соблюдение бюджетов перечислимости. Комментарии внутри кода поясняют выбор констант и указывают на связь с `coverThreshold`.
...
- [x] Внести при необходимости мелкие правки в документацию или комментарии, описывающие новую теорему, и обновить ссылки на соответствующие разделы статьи/препринта.
  *Статус:* комментарии в `Pnp2/Circuit/EntropyCover.lean` и настоящий план синхронизированы; ссылки на модули `Circuit.EntropyCover` и `Circuit.Numeric` обновлены после итогового рефакторинга.
