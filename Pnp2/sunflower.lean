...
lemma exists_of_large_family
  ...
  classical
  rcases sunflower_exists F w t (by simp [hcard]) (by intro ht; exact hbig ht)
    with ⟨pet, hsub, core, hsize, hpair, hsubcore⟩
...
noncomputable def sunflowerStep : Σ' (C : Petal n), Subcube n := by
  ...
  let fam : Finset (Petal n) :=
    U.image fun x => support (F.choose x hFnonempty)
  have hcard : ∀ S ∈ fam, S.card = w := by
    intro S hS
    rcases Finset.mem_image.1 hS with ⟨x, hx, rfl⟩
    have hxF : (F.choose x hFnonempty) ∈ F := by classical simpa
  ...
  refine ⟨S.core, sunflowerSubcube S.core (U.choose hU)⟩
