...
theorem antiChecker_exists_large_Y
  ...
  ∃ (F : Family (Models.inputLen p))
    (Y : Finset ...), ...
```
Получается из аксиомы `antiChecker_exists_large_Y` (базовый античекер без
тест-набора) и поэтому не требует новых предположений.
