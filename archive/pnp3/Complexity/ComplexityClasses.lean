...
доступны в архивной библиотеке.
...
**Конкретное**: Полное определение через TM есть в архивной библиотеке ComplexityClasses.lean
-/
def InP (L : Language) : Prop :=
  -- Абстрактно: "есть полиномиальный алгоритм"
  -- Конкретно (в архивной библиотеке): ∃ (T : TM) (c : ℕ), ... runtime ≤ n^c ...
...
**Конкретное**: Полное определение через TM verifier в архивной библиотеке 
-/
def InNP (L : Language) : Prop :=
  -- Абстрактно: "есть poly-time verifier с poly-size certificate"
  -- Конкретно (в архивной библиотеке): ∃ (k c : ℕ) (T : TM), ... verifier ...
...
**Конкретное**: Полное определение через circuit families в архивной библиотеке 
-/
def InPpoly (L : Language) : Prop :=
  -- Абстрактно: "есть poly-size circuit family"
  -- Конкретно (в архивной библиотеке): ∃ circuits, size ≤ poly(n), circuits correct
...
**Конкретное доказательство**: См. архивной библиотеке PsubsetPpoly.lean (конструктивное!)
...
1. **Modularity**: Не нужно дублировать всю TM/circuit infrastructure из архивной библиотеке
...
**If needed**: Полное конструктивное доказательство есть в архивной библиотеке PsubsetPpoly.lean
