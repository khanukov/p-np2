...
3. **Выбор чувствительного бита**
 ...
 лемма `glue_branch_coversPw`, нормализующая покрытия ветвей и объединяющая
  ...
  Реализация `buildCoverLex3` теперь действительно ветвится по чувствительной
  координате: для каждой ветви рекурсивно вызывается `buildCoverLex3A` с
  обновлённым набором координат `A.erase i`, после чего результаты склеиваются
  через `glue_branch_coversPw_mBound`. Тест
  `buildCoverLex3` в `CoverResBaseTest.lean` демонстрирует корректность этой
  рекурсивной стратегии на примере одноразрядной функции.
...
6. **Оптимизация стиля**  
  ...
  7. **Гарантия терминирования**
    *Сделано:* тройная мера `measureLex3` обеспечивает строгий спад как при
    ...
    блоке `decreasing_by` для обеих ветвей.
...
## Обновление плана (рефакторинг `buildCoverLex3`)
  ...
  1. **Рекурсивное ветвление по чувствительной координате.**
    - Первоначально в `buildCoverLex3` чувствительный случай обрабатывался через
      заглушку `pointCover_succ`.
    - Теперь ветвь реализована рекурсивными вызовами `buildCoverLex3A` на обеих
      ветвях с последующей склейкой `glue_branch_coversPw_mBound`. Определения
      `glue_step_pw`, `glue_branch_coversPw` и `glue_branch_coversPw_mBound`
      подняты выше по файлу, чтобы ими можно было воспользоваться.
    - Лемма `glue_branch_coversPw_card_le_mBound_succ`, ранее служившая
      заглушкой, удалена.
   - ✅ Блок `decreasing_by` содержит только случай удаления постоянно ложной функции; фиксация координаты не вызывает рекурсии.
   - ✅ Введена вспомогательная функция `buildCoverLex3A` с параметром множества координат `A`.
     Обёртка `buildCoverLex3` вызывает её с `A = univ`, что подготавливает инфраструктуру
