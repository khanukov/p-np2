...
4. **Переход от покрытия к дереву решений.**
    ...
    - Попытка ослабить условие монохроматичности до точечного уровня показала, что текущая реализация `DecisionTree.ofRectCover` принимает только семейно‑монохроматичные прямоугольники. Требуется либо расширить API дерева, либо оставить сильную формулировку `Subcube.monochromaticForFamily`.
