...
`buildCoverLex3` полностью реализует ветвление: сначала устраняются
тождественно `false` функции (`CoverResP.lift_erase_false`), затем выбирается
чувствительная координата, и для каждой ветви рекурсивно строится покрытие.
Склейка осуществляется через `glue_branch_coversPw_mBound`, а леммы
`measureLex3_restrictDrop_univ_lt` фиксируют спад меры в обеих ветвях.
Тест `buildCoverLex3` в `CoverResBaseTest.lean` демонстрирует корректность
рекурсии на примере одноразрядной функции.
...
7. **Гарантия терминирования**
   *Сделано:* подготовлены леммы, обеспечивающие положительность меры и её
   ...
   функцию (через `measureLex3_erase_lt` и её экзистенциальную форму
   `exists_measureLex3_drop_const_false`), либо фиксирует чувствительный бит и
   уменьшает размер множества `A`.  Введены операции `restrictDrop` и
   `extendDrop` с леммами `measureLex3_restrictDrop_lt`,
   `measureLex3_restrictDrop_univ_lt` и
   `exists_branch_measure_drop_restrictDrop_univ`, фиксирующими строгий спад
   меры при ветвлении; в коде `buildCoverLex3` эта информация теперь явно
   сохраняется для обеих ветвей.
   *Сделано:* ветвление на основе `restrictDrop` встроено в `buildCoverLex3`.
   Оба подсемейства строят покрытия рекурсивно и склеиваются при помощи
   `glue_branch_coversPw_mBound`, что оформлено в виде трёхкомпонентной меры.
   *Осталось:* довести общий алгоритм до теоремы `decisionTree_cover` и
   построить окончательное дерево решений.
...
1. **Связь чувствительности с уменьшением меры.**
    ...
    - ✅ Ветвление реализовано через `restrictDrop` и
      `glue_branch_coversPw_mBound`, каждая ветка сопровождается фиксированным
      уменьшением `measureLex3`.
...
1. **Рекурсивное ветвление по чувствительной координате.**
   ...
   - Для полноценной рекурсии требуется:
    - ✅ Переместить определения `glue_step_pw`, `glue_branch_coversPw` и `glue_branch_coversPw_mBound` выше по файлу, чтобы они были доступны при определении `buildCoverLex3`.
    - ✅ Ветвь `hsens` извлекает чувствительную координату `i`, рекурсивно вызывает `buildCoverLex3` на подсемействах `F.restrict i false/true` и склеивает результаты через `glue_branch_coversPw`. Инстансы `[Fintype (Point n)]` унифицированы через `letI := inferInstance`, оценки `coordSensitivity` получены с помощью `by simpa`.
    - ⚠️ Добавлена вспомогательная лемма `glue_branch_coversPw_card_le_mBound_succ`, связывающая склейку ветвей с требуемой оценкой `≤ mBound n (h + 1)`.  В текущей формулировке каждая ветка уже ограничена `mBound n (h + 1)`, что делает доказательство невозможным: из `2 * mBound n (h + 1)` нельзя вывести `mBound n (h + 1)`.  Необходимо переработать лемму так, чтобы ветвям выделялся бюджет `mBound n h`, после чего используется `Cover2.two_mul_mBound_le_succ`.
    - ⏳ Обосновать спад `measureLex3` в блоке `decreasing_by` для обеих ветвей.  Сейчас в доказательстве остаются две заглушки `admit`; требуется увязать `measureLex3_drop_coord` и `measureLex3_restrictDrop_univ_lt`, чтобы показать уменьшение тройной меры после фиксации координаты.
   - Ключевая трудность — корректный аргумент о терминации: необходимо показать спад `measureLex3` для обеих ветвей. Лемма `measureLex3_restrictDrop_univ_lt` даёт уменьшение меры при переходе к `A.erase i`, но требует увязать её с измерением, используемым в рекурсивных вызовах.
   - План действий: уточнить функцию меры в `termination_by`, чтобы учесть уменьшенное множество координат, или предоставить дополнительную лемму, связывающую меру ветви, рассчитанную на `Finset.univ`, с уменьшенной мерой из `measureLex3_restrictDrop_univ_lt`.
