...
1. **Мера для индукции.**  Вводим функцию
   ...
   * Вопрос: какую нормировку использовать при переходе от вещественного
     `H₂(F)` к натуральному `measure F`?  Возможно, потребуется умножение
     на `(n+1)^k` или взятие `Nat.ceil`.
...
3. **Выбор чувствительной координаты.**  Если семейство не постоянно,
   ...
   * Вопрос: необходимо формально доказать вспомогательное утверждение
     ...
     В `Pnp2/BoolFunc/Sensitivity.lean` уже есть лемма
     `exists_family_sensitive_coord`, которую можно использовать после
     вывода факта `∃ f ∈ F, 0 < sensitivity f`.  Требуется отдельная
     связь между «семейство не постоянно» и наличием функции с
     положительной чувствительностью.
