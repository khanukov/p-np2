...
1. **Рекурсивное ветвление по чувствительной координате.**
   ...
   - Для полноценной рекурсии требуется:
    ...
    - ⚠️ Добавлена вспомогательная лемма `glue_branch_coversPw_card_le_mBound_succ`, связывающая склейку ветвей с требуемой оценкой `≤ mBound n (h + 1)`. В лемме реализована оценка объединения ветвей через `≤ 2 * mBound n (h + 1)`, однако переход к окончательной границе ещё не доказан (`admit`).
    - ⏳ Обосновать спад `measureLex3` в блоке `decreasing_by` для обеих ветвей.
