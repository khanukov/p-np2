...
1. **Комбинаторная оценка числа субкубов.**
    - В `Cover/Bounds.lean` добавлена лемма `card_subcube_le_mBound`,
      показывающая `Fintype.card (Subcube n) ≤ mBound n h` при условии
      `n ≤ 5⋅h`.  В `buildCoverLex3` базовый случай теперь опирается на
      эту оценку.
    - Остаётся снять дополнительное ограничение `n ≤ 5⋅h` или показать,
      что оно автоматически выполняется в применении теоремы.
...
3. **Рекурсивное построение покрытия.**
    ...
    - **Текущее препятствие:** попытка сменить тип возвращаемого значения уже показывает, что существующие вспомогательные леммы построены для `Finset`‑покрытий; при переходе на `CoverRes` появляются десятки несоответствий типов и отсутствующие леммы про `card`, `mono` и `covers`.
      Требуется последовательная переработка: сначала адаптировать базовые случаи, затем переписать рекурсивный шаг с `glue_branch_covers`, и лишь после этого переносить результирующие свойства в основную теорему.
