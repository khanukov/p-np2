...
`theorem` с пока недостающим доказательством (`sorry`).
...
- Определение `CoverRes.toDecisionTree` преобразует покрытие в дерево решений;
  леммы `CoverRes.leaf_count_le`, `CoverRes.depth_le_card_mul` и
  `CoverRes.depth_le_coverConst` связывают параметры дерева с оценкой размера
  покрытия.
- Все регрессионные примеры собраны в модуле `test/CoverResBaseTest.lean`,
  который подключён к `lake test`.
- Принято решение сохранять требование семейно‑монохроматичных прямоугольников,
  поэтому API `DecisionTree` не меняется.
...
3. **Выбор чувствительного бита**
   *Сделано:* лемма `non_constant_family_has_sensitive_coord` находит
   ...
   подготовлены базовые случаи `CoverRes.empty` и `CoverRes.const`, а также
   лемма `glue_branch_covers`, нормализующая покрытия ветвей и объединяющая их.
   *Осталось:* переписать `buildCoverLex3`, чтобы рекурсивный шаг сразу
   возвращал `CoverRes` и вызывал `glue_branch_covers` вместо простого
   объединения списков субкубов.

4. **Монадическое построение дерева**
   *Сделано:* разработана стратегия: сначала строится покрытие субкубами, затем
   оно переводится в `DecisionTree`. Введена функция `CoverRes.toDecisionTree` и
   доказаны леммы `CoverRes.leaf_count_le`, `CoverRes.depth_le_card_mul`,
   `CoverRes.depth_le_coverConst` и `CoverRes.eval_true`, демонстрирующие
   корректность дерева и его связь с мощностью покрытия.
   *Осталось:* использовать построенное дерево в доказательстве
...
- ✅ Определение `CoverRes.empty` задаёт тривиальное покрытие для пустого
...
- ✅ Определение `CoverRes.const` строит базовое покрытие из одного полного
...
- В покрытии будем требовать **семейно‑монохроматичные** прямоугольники; API `DecisionTree` остаётся прежним и маркирует лист единственным булевым цветом.
- Инвариант `BuildInv` менять не нужно: рекурсивная функция `buildCoverLex3` будет возвращать структуру `CoverRes` и использовать `glue_branch_covers` для склейки ветвей.
...
2. **Связь чувствительности с уменьшением меры.**
    - Пересмотреть лемму `exists_restrict_eq_of_sensitive`; если она не даёт половинного уменьшения семейства, полагаться на убывание по третьей компоненте `measureLex3`.
    - На основе новой формулировки показать спад первых двух компонент `measureLex3` и обновить соответствующие доказательства.
...
4. **Переход от покрытия к дереву решений.**
    ...
    - Поскольку мы оставляем сильное условие `Subcube.monochromaticForFamily`, каждый лист дерева помечается единым цветом, и расширение API не требуется.
