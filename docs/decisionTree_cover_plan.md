...
- Существует каркас для объединения покрытий (`merge_low_sens.lean`) и
  зафиксирована безопасная константа `C = 10` для всех асимптотических оценок.
...
2. **Строгое уменьшение меры**  
   *Сделано:* леммы `measure_restrict_le`, `measure_restrict_lt_of_card_le_half`
   и `measure_filter_lt_of_card_le_half` показывают падение меры при
   существенном уменьшении семейства.  
   *Осталось:* доказать, что при разбиении по чувствительной координате одна из
   ветвей уменьшает меру, либо перейти к лексикографической мере
   `(measure F, card F)`.
...
1. В `entropy.lean` завершить лемму `measure_restrict_decreases` или внедрить
   лексикографическую меру.
2. Подготовить вспомогательные факты для склейки покрытий и оценки количества
   субкубов.
3. Переписать `decisionTree_cover` в `low_sensitivity_cover.lean` как
   рекурсивную процедуру, использующую найденную чувствительную координату и
   ограничение меры.
4. Построить дерево из полученного покрытия (`DecisionTree.ofRectCover`) и
   связать глубину с мерой через `leaf_count_le_pow_depth`.
5. Удалить аксиому, обновить `TODO.md` и выполнить `./scripts/check.sh`.
