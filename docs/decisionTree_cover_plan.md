...
- В `coloredSubcubesAux_cons_subset_node_perm` реализован индуктивный перенос
  головного присваивания через префикс и единственный обмен с последним
  вхождением `j`.  Заключительный шаг с восстановлением исходного порядка
  и переносом включения подкубов остаётся незавершённым.
- В качестве подготовки добавлена вспомогательная лемма
  `not_mem_idx_of_mem_left_of_nodup_append`, показывающая, что индекс,
  встречающийся в префиксе пути без повторов, не может появиться в
  хвосте.  Она пригодится при формальном доказательстве свежести
  координат при перестановках.
- В `exists_common_monochromatic_subcube` устранён первый пропуск:
  чувствительность выбранной функции при вызове `huang_step`
  выводится напрямую из глобального условия `Hsens`, что ликвидирует
  один `sorry` в определении шага.
...
4. **Оценка числа прямоугольников и глубины.**
    ...
    - Малый случай `s ≤ n + 1` в теореме `decisionTree_cover` теперь
      покрывается тривиальным набором всех точечных подкубов; соответствующий
      `sorry` в `low_sensitivity_cover.lean` устранён.
...
3. **Интеграция в `decisionTree_cover`.**
   ...
   `R.idx.card ≤ coverConst · s`.  Для семейств, содержащих две и более
   функций, перенос монохроматичности на остальные элементы остаётся
   незавершённой задачей.
    Вокруг выбранного подкуба формируется покрытие
    `branchOnSubcube`, а внешняя область теперь описывается чисто
    геометрически: лемма `cover_with_common_cube` объединяет сам подкуб `R`
    с покрытием точек `¬ R.mem`.  Рекурсивная лемма
    `decisionTree_cover_smallS_pos_general_rec` больше не используется.
    Обеспечение покрытия целиком берёт на себя общая лемма
    ...
    В `coloredSubcubes_branchOnSubcube_subset` уже полностью доказан
    вырожденный случай `R.idx = ∅` — показано, что единственная ветвь
    совпадает с `R`; общий индуктивный разбор реализован частично:
    обработан переход вдоль основной ветви, но остаётся исключить
    вырожденное совпадение подкуба с исходным.  В качестве
    ...
    координат подкуба.  Дополнительно доказана базовая версия
    `not_mem_subcube_exists_mismatch`, возвращающая координату с несовпадающим
    битом, а также усиленная лемма
    `not_mem_subcube_exists_mismatch_eq`, позволяющая сразу получить
    координату с перевёрнутым битом для любой точки вне подкуба.  Оценка
    глубины напрямую
    ...
    используется при анализе включений между подкубами.  Дополнительно
    введена эквивалентность
    `DecisionTree.mem_subcube_of_path_cons`, связывающая принадлежность
    расширенному пути с выполнением нового присваивания и членством в исходном
    подкубе.  Это упрощает последовательные перестановки и удаление
    присваиваний.
    Появилась также утилита
    `DecisionTree.mem_subcube_of_path_cons_subset_of_not_mem_fst`, которая
    позволяет использовать лемму про включение, опираясь лишь на отсутствие
    координаты в списке `p.map Prod.fst`.
    ...
    * Для пустого хвоста пути уже реализована специализированная версия
    ...
    используется напрямую через вычисление `|Point n| = 2^n`.  Рекурсивная
    лемма больше не применяется: внешний подкуб покрывается напрямую
    конструкцией `cover_with_common_cube`.  Тем не менее перенос
    монохроматичности на остальные функции семейства остаётся открытым,
    поэтому вводится промежуточное утверждение
    `hins_all : ∀ f ∈ F, coordSensitivity f i = 0`,
    позволяющее при его наличии вывести монохроматичность `R₁` через
    `Subcube.monochromaticFor_unfix_of_insensitive`.  Формальное доказательство
    `hins_all` по-прежнему отсутствует, как и вывод из него монохроматичности `R₁`
    и проверка глубины после ветвления пока оставлены `sorry`.  При этом
    равенство
