...
выбора.
Дополнительно мы свободно применяем следующие приёмы:

- открываем `Classical` и работаем в `noncomputable`-секциях, если это упрощает
  доказательство;
- используем стандартные аксиомы mathlib (`funext`, `propext`,
  `Classical.choice`) без дополнительных оговорок;
- вводим неконструктивные определения, когда требуется лишь утверждение в
  `Prop`, а не вычислимый объект;
- опираемся на уже формализованные леммы mathlib, включая факты о
  вычислимости, автоматах, деревьях решений и т.п., без попытки заново
  строить конструктивные доказательства.
...
- Для удобства работы с путями добавлена вспомогательная лемма
  `nodup_map_fst_cons`, подтверждающая сохранение `Nodup` при добавлении
  новой координаты. Это упростит последующие доказательства включений между
  подкубами, где требуется свежесть индексов.
- Разработана вариация `nodup_map_fst_cons_of_not_mem_idx`, позволяющая
  выводить `Nodup` из отсутствия координаты в наборе `idx` подкуба; она
  используется при переносе включений из цветных подкубов дерева решений в
  исходный путь.
- Добавлена обратная лемма `not_mem_map_fst_of_cons_nodup`, позволяющая из
  `Nodup` для расширенного пути `(i, b) :: p` заключить, что координата `i`
  отсутствовала в исходном списке индексов `p.map Prod.fst`.
- Введена лемма `mem_path_of_mem_subcube_idx`, позволяющая по
  принадлежности индекса подкубу восстановить его появление в исходном
  пути; это полезно при переносе свойств между путями и соответствующими
  подкубами.
- Добавлена лемма `not_mem_path_of_not_mem_subcube_idx`, показывающая,
  что отсутствие координаты в наборе `idx` подкуба гарантирует её
  отсутствие и в исходном пути; используется для явного контроля
  свежести индексов при расширении путей.
...
- В самой теореме `decisionTree_cover` теперь явно разбираются тривиальные
  ...
  применяется лемма `mBound_le_pow_of_budget_choice_bigS`.  Для режима
  `s ≤ n + 1` подготовлены отдельные конструкции: `decisionTree_cover_smallS_zero`
  обрабатывает нулевую чувствительность, `decisionTree_cover_smallS_pos_n1`
  покрывает измерение `n = 1`, а `decisionTree_cover_smallS_pos_general`
  интегрирует геометрическую склейку `cover_with_common_cube`.  Итоговая
  обёртка `decisionTree_cover_smallS` подключается в доказательстве теоремы.
  Однако ключевая лемма `exists_common_monochromatic_subcube`, обеспечивающая
  общий монохроматический подкуб, пока содержит две заглушки `sorry`
  (глобальная монохроматичность и оценка `R.idx.card ≤ coverConst * s`).
  Из-за этого вся ветка `s ≤ n + 1` остаётся незавершённой, хотя остальные
  части доказательства уже прописаны.
...
- ✅ Завершена лемма `subcube_of_path_eq_self`, восстанавливающая подкуб из
  спискового представления и устраняющая соответствующий `sorry`.
  **Файл:** `DecisionTree.lean`.

- В лемме `subcube_of_coloredSubcubesAux_le_subcube_of_path` добавлены
  развёрнутые комментарии, описывающие необходимость доказательства
  свежести корневого индекса.  Завершение этого шага зависит от будущей
  леммы `coloredSubcubesAux_cons_subset_node_perm`, обеспечивающей
  перестановку повторных координат в пути.
...
4. **Оценка числа прямоугольников и глубины.**
    ...
    - Завершить малый случай `s ≤ n + 1` в теореме `decisionTree_cover`,
      заполнив два `sorry` в `exists_common_monochromatic_subcube`.
...
3. **Интеграция в `decisionTree_cover`.**
   Полученное дерево преобразуется в покрытие посредством
    ...
    `decisionTree_cover_smallS_pos_general` теперь строится через лемму
   `exists_common_monochromatic_subcube`.  Она должна вернуть подкуб `R`,
   монохроматичный для каждой функции семейства, и ограничить
   `R.idx.card ≤ coverConst · s`.  Из этих данных лемма
   `cover_with_common_cube` формирует покрытие `R ∪ coverExterior`, а
   глобальный результат извлекается через `decisionTree_cover_of_coverResP`.
   В текущей реализации оба шага, отвечающие за `exists_common_monochromatic_subcube`,
   ещё пустуют: необходимо доказать семейную монохроматичность `R` и его
   ограниченную кодименсию.  Остальная часть `decisionTree_cover_smallS_pos_general`
   уже проверяет покрытие и численную оценку, так что закрытие этих двух
   задач автоматически завершит ветку малого `s`.

    На стороне деревьев решений продолжается работа с цветными подкубами.
    Леммы `coloredSubcubesAux_cons_cons`, `coloredSubcubesAux_cons_swap`,
    `coloredSubcubesAux_cons_subset_node_same` и
    `coloredSubcubesAux_cons_subset_node_diff` доказаны, но общий случай
    `coloredSubcubesAux_cons_subset` всё ещё требует перестановки координат в
    хвосте пути.  Конечная перестановка сведена к двум незакрытым леммам:

    * `coloredSubcubesAux_cons_bubble` (перемещение `(j, bj)` через префикс).
    * `coloredSubcubesAux_cons_subset_node_perm`, использующая `bubble` для
      разруливания пересечений координат и обеспечивающая включение нужного
      подкуба.

    Без них общие утверждения `coloredSubcubesAux_cons_subset` и
    `coloredSubcubes_branchOnSubcube_subset` остаются с `sorry`, что блокирует
    дальнейший анализ глубины дерева.

    Следующие шаги:

    1. Доказать оба оставшихся шага в `exists_common_monochromatic_subcube`
       (семейная монохроматичность и оценка `R.idx.card ≤ coverConst · s`).
    2. Завершить `coloredSubcubesAux_cons_bubble` и
       `coloredSubcubesAux_cons_subset_node_perm`, а затем вывести полный
       вариант `coloredSubcubesAux_cons_subset`.
    3. Перенести эти результаты в `coloredSubcubes_branchOnSubcube_subset`
       и `coloredSubcubes_branchOnSubcube_monochromatic`, чтобы полностью
       закрыть геометрический шаг `cover_with_common_cube`.
