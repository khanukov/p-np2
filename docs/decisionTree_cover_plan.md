...
3. **Интеграция в `decisionTree_cover`.**
   ...
   незавершённой задачей.
    ...
    координат подкуба.  Также введена лемма `not_mem_subcube_exists_mismatch_eq`,
    позволяющая сразу получить координату с перевёрнутым битом для любой точки
    вне подкуба.  Оценка глубины напрямую
