...
1. **Связь чувствительности с уменьшением меры.**
    ...
    - Добавлена лемма `measureLex3_drop_coord`, показывающая, что исключение
      ...
      семейства.
    - В чувствительной ветви `buildCoverLex3` планируется использовать
      рекурсивное ветвление, однако сейчас эта часть остаётся заглушкой:
      обнаружив чувствительную координату, функция возвращает тривиальное
      `pointCover` для всего семейства.  Леммы `glue_branch_coversPw` и
      `glue_branch_coversPw_mBound`, необходимые для будущей склейки ветвей,
      уже подготовлены, но пока не задействованы.
    - Добавлены обёртки `CoverResP.pointCover_succ`, `CoverResP.const_mBound`
      ...
      следующего уровня `mBound`.  Текущая реализация `buildCoverLex3`
      использует лишь базовый `pointCover`, подготавливая почву для
      полноценного рекурсивного ветвления.
...
1. **Рекурсивное ветвление по чувствительной координате.**
   ...
   - ℹ️ Ветвь `hsens` пока возвращает простое `pointCover` для всего семейства;
     подготовленная инфраструктура `glue_branch_coversPw_mBound` ещё не
     используется.
   - 🔄 Следующий шаг — заменить эту заглушку на рекурсивные обращения к
     `buildCoverLex3` для обеих ветвей с последующей склейкой.
