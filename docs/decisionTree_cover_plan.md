...
1. **Связь чувствительности с уменьшением меры.**
    ...
    - Осталось заменить заглушку `pointCover` на реальное ветвление с
      использованием `glue_branch_coversPw`, чтобы каждая ветка рекурсии
      опиралась на доказанное уменьшение `measureLex3`.
    - Наивная попытка склеить две точечные ветви через `glue_branch_coversPw`
      приводит к удвоению оценки мощности (`k₀ + k₁`), которая уже не
      укладывается в глобальную границу `mBound n h`.  Необходимо либо
      вызывать рекурсивный шаг, который сам обеспечивает меньший размер ветви,
      либо доказать усиленную оценку `mBound`.
2. **Рекурсивное построение покрытия.**
    ...
    - Встроить нормализацию ветвей через `cover_normalize_branch_pointwise`,
      реализовать чувствительное ветвление через `glue_branch_coversPw` и при
      необходимости определить варианты `restrictDrop` / `extendDrop` с явным
      убыванием меры.
