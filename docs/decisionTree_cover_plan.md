...
3. **Интеграция в `decisionTree_cover`.**
   ...
   незавершённой задачей.
    ...
    фиксирование той же координаты на уровне индексов).
    ...
    будущей доработки.  Оценка глубины напрямую
    ...
    реализация рекурсивного шага `hRec`.  В непустом случае теперь явно
    выделяется вторая функция `f₁ ≠ f₀`, что подготовит перенос монохроматичности
    с опорной функции на всё семейство.  Соответствующий анализ остаётся
    помеченным `TODO`.
