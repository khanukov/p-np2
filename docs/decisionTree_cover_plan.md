 ...
 *Сделано:* леммы `measure_restrict_le`, `measure_restrict_decreases`,
 ...
 спад меры при выборе чувствительной координаты.
  *Осталось:* интегрировать эти оценки в рекурсивный алгоритм построения
 покрытия, чтобы на каждом ветвлении фиксировать уменьшение `measureLex3` и
 получать более сильные границы на размер покрытий.
...
ветвлению.  Конструкция `CoverResP.pointCover` перечисляет все точки куба как
нулевые субкубы и даёт явную цепочку оценок `2^n ≤ 3^n ≤ mBound n h`,
подтверждая ограничение мощности покрытия в базовом случае.
Тест `buildCoverLex3` в `CoverResBaseTest.lean` демонстрирует корректность
этого базового шага на примере одноразрядной функции.
  🔧 В `buildCoverLex3` теперь реализован шаг предварительной фильтрации:
  тождественно `false` функции удаляются с помощью `CoverResP.lift_erase_false`,
  и рекурсия продолжается на уменьшенном семействе.
 ✅ `buildCoverLex3` переписан в терминах `CoverResP` и уже использует
  `glue_branch_coversPw` для объединения ветвей; некоторые доказательства
...
2. **Связь чувствительности с уменьшением меры.**
    ...
    - Осталось встроить эти результаты в рекурсивный алгоритм `buildCoverLex3`, обеспечив фиксируемое уменьшение меры на каждом шаге.
3. **Рекурсивное построение покрытия.**
    - ✅ `buildCoverLex3` теперь возвращает структуру `CoverResP`; дальнейшая работа
      включает интеграцию `glue_branch_coversPw` и нормализацию ветвей
      `cover_normalize_branch_pointwise`.
    - Встроить нормализацию ветвей через `cover_normalize_branch_pointwise` и при необходимости определить варианты `restrictDrop` / `extendDrop` с явным убыванием меры.
