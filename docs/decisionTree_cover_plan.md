...
- В лемме `subcube_of_coloredSubcubesAux_le_subcube_of_path` добавлены
  ...
  перестановку повторных координат в пути. В текущей реализации зафиксирован
  каркас доказательства этой леммы: выделен последний вход индекса и
  подготовлен базовый случай, когда префикс пуст. Далее потребуется
  аккуратная индукция по длине префикса и перенос доказательства включения
  подкубов через `coloredSubcubesAux_cons_swap`.
...
3. **Интеграция в `decisionTree_cover`.**
   ...
   незавершённой задачей.
    ...
    совпадает с `R`; общий индуктивный разбор остаётся TODO.  В качестве
