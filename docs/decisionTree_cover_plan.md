...
формально доказывает монохроматичность этого подкуба.  Вместе эти факты
даёт готовую конструкцию крупного монохромного блока вокруг любой точки.
...
`x` всегда лежит в своём `insensitiveSubcubeAt`.
На глобальном уровне показано, что
`sensitivity f = 0` эквивалентно пустой поддержке `support f`.
...
`exists_large_insensitive_subset_div`, которая заменит текущую более грубую
`exists_large_insensitive_subset` и станет ключом к построению дерева глубины
`O(s·log n)`.
Частичный результат уже достигнут в виде
`exists_large_insensitive_subset_div_of_small`, требующей дополнительного
численного условия и покрывающей малые случаи.
...
1. **Большой блок нечувствительных координат.**
   Нужно доказать усиленную лемму `exists_large_insensitive_subset`,
   обеспечивающую подмножество `A ⊆ Fin n` размера `|A| ≥ n / (2*s)` такое,
   что значение `f` не меняется при одновременном перевороте любого поднаборa `A`.
   Уже получена более слабая версия этой леммы: `exists_large_insensitive_subset`
   конструирует множество глобально нечувствительных координат мощности
   `≥ n - 2^n * s`, на котором `f` остаётся константной.  Однако полученная
   оценка слишком груба и даёт полезную информацию лишь для малых `n`.
   Следующий шаг — усилить bound до требуемого `n / (2*s)`.
...
3. **Интеграция в `decisionTree_cover`.**
   ...
   `decisionTree_cover_of_tree`, после чего временная аксиома
   `decisionTree_cover_smallS_pos_general` удаляется.
