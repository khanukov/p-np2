...
- ✅ Получены первые оценки числа листьев: леммы
  `leaf_count_matchSubcube` и `leaf_count_ofRectCoverList_le` связывают
  количество листьев построенного дерева с длинами списков заданных
  субкубов.  Дополнительно получены локальные оценки глубины — леммы
  `depth_matchSubcube_le` и `depth_ofRectCoverList_le` ограничивают число
  последовательных проверок.  Связь с глобальной оценкой `DecisionTree.leaf_count_le_pow_depth`
  пока остаётся открытым пунктом.  **Файл:** `DecisionTree.lean`.

### 5) Глобальная оценка глубины

- *Сделано:* получены вспомогательные леммы `depth_matchSubcube_le` и
  `depth_ofRectCoverList_le`, дающие локальные оценки глубины при
  построении дерева из покрытия.
- *Осталось:* собрать глобальную оценку числа листьев и перевести её в
  ограничение на глубину `depth ≤ C · s · log₂(n+1)`.  **Файл:**
