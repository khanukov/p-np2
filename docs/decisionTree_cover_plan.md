...
- Определение `CoverResP.toDecisionTree_for` преобразует покрытие в дерево
 ...
 - Все регрессионные примеры собраны в модуле `test/CoverResBaseTest.lean`,
    ...
    `CoverResP.toDecisionTree_for` на константной семье, новый тест
    демонстрирует работу `CoverResP.lift_erase_false` при возвращении
    тождественно ложной функции в семейство, а свежий пример проверяет
    склейку ветвей через `glue_branch_coversPw` на одноразрядной
    идентичной функции.
