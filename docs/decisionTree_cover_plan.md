...
3. **Выбор чувствительного бита**
   *Сделано:* лемма `non_constant_family_has_sensitive_coord` находит
   ...
   В `buildCoverLex3` добавлен защитный базовый случай: покрытие всеми
   субкубами используется только при `n ≤ 5⋅h`, иначе рекурсия переходит к
   ветвлению.
   ✅ `buildCoverLex3` переписан в терминах `CoverRes` и уже использует
   `glue_branch_covers` для объединения ветвей; некоторые доказательства пока
   остаются заглушками.
...
8. ✅ Введена тройная лексикографическая мера `measureLex3 F A =
  ...
  🔧 В `buildCoverLex3` перед ветвлением удаляются тождественно ложные функции,
   ...
   нормализуются и склеиваются при помощи `glue_branch_covers`. Для перехода
   от подсемейства обратно к исходному добавлена лемма
   `CoverRes.lift_erase_false`, наследующая покрытие после удаления
   константно ложной функции. *Осталось:* закрыть `admit` в доказательстве
   монохроматичности при подъёме и провести анализ операций
...
- ✅ Определение `CoverRes.const` строит базовое покрытие из одного полного
  ...
  **Файл:** `low_sensitivity_cover.lean`.
  ...
  ✅ `buildCoverLex3` переписан так, что возвращает заготовку `CoverRes`;
  последующие версии встроят вызовы `glue_branch_covers` для склейки ветвей
  и доведут доказательства монохроматичности, покрытия и оценки мощности.
...
1. **Комбинаторная оценка числа субкубов.**
    - В `Cover/Bounds.lean` зафиксирована лемма `card_subcube_le_mBound` с
      условием `n ≤ 5⋅h`, дающим оценку `Fintype.card (Subcube n) ≤ mBound n h`.
      Базовый случай `buildCoverLex3` опирается на эту слабую, но
      достаточную границу; усиливать её не планируется.
...
3. **Рекурсивное построение покрытия.**
    - ✅ `buildCoverLex3` теперь возвращает структуру `CoverRes`; дальнейшая работа
      включает интеграцию `glue_branch_covers` и нормализацию ветвей
      `cover_normalize_branch`.
