...
- [ ] В существующем модуле (например, `cover_numeric.lean` или новом файле `Cover/Circuits.lean`) ввести определение семейства $\mathcal F_{n,c}$ всех булевых функций на $n$ переменных, реализуемых схемами размера $\le n^c$. При необходимости использовать уже существующий конструктор `canonicalCircuit` для получения Lean-объектов схем.
- [ ] Убедиться, что определение согласовано с имеющимися понятиями о схемах и булевых функциях (например, `Circuit.eval`, `BoolCube`). Проверить, что семейство представлено как `Finset` или `Set` (в зависимости от интерфейса `family_collision_entropy_lemma`). При несовпадении типов подготовить вспомогательную конверсию.
  *Статус:* после отката коммита `e239d05` определения `Circuit.family` и `Circuit.powFamily` отсутствуют; требуется восстановить семейство схем и согласовать его с текущей канонизацией.

1.2. Получить верхнюю оценку на мощность семейства
- [ ] Использовать доказанную лемму `count_canonical_bounded` (или более прямой аналог) для вывода оценки $|\mathcal F_{n,c}| \le 2^{C n^{c+1} + 1}$ для некоторой явной константы $C$. Проверить, что константа совместима с ограничениями на параметр $c$ и предполагаемыми границами на $n$.
- [ ] Сформулировать и доказать в Lean промежуточную теорему (например, `family_of_circuits_card_bound`), возвращающую константу $C$ и требуемое неравенство для всех достаточно больших $n$. В этой лемме явно использовать `Bound.family_card_le` или аналогичные результаты, чтобы показать совместимость нового определения семьи с существующими оценками.
- [ ] Из неравенства на мощность вывести ограничение на коллизионную энтропию: $H_2(\mathcal F_{n,c}) \le C n^{c+1} + 1$. Зафиксировать явную связь между доказанным `card`-неравенством и формализацией энтропии (включая преобразование логарифмов).
  *Статус:* леммы `Circuit.pow_family_card_le` и `Circuit.pow_family_H₂_le` были удалены вместе с модулем `Circuit/EntropyCover`; требуется повторная реализация оценок мощности и энтропии.

1.3. Подготовить параметры для применения `Bound.family_collision_entropy_lemma`
- [ ] Записать в Lean вспомогательную лемму, связывающую выбранные $C$ и $h(n) = C n^{c+1} + 1$ с гипотезами `family_collision_entropy_lemma`, включая проверку свойств натуральных логарифмов, преобразование `Nat.cast` и условия $0 < n$.
- [ ] Проверить, что необходимые условия (монотонность $h$, диапазоны индексов, неотрицательность) явно зафиксированы и легко доступны в доказательствах. При необходимости сформулировать отдельные леммы о росте $h$ и совместимости с экспонентами.
  *Комментарий:* после удаления прежней конструкции `powFamilyEntropyBound` параметры $h$ и связанные преобразования нужно построить заново.

## 2. Получение покрытия с нужной мощностью
2.1. Применение `family_collision_entropy_lemma`
- [ ] Для $\mathcal F_{n,c}$ вызвать `Bound.family_collision_entropy_lemma`, подставив вычисленное $h(n)$, и убедиться, что все вспомогательные параметры (натуральные логарифмы, условия на размер семейства) удовлетворены автоматически.
- [ ] Извлечь из результата множество подкубов $Rset$ и границу на его мощность $|Rset| \le 2^{3n + 11 h(n) + 2}$. При необходимости добавить вспомогательные леммы, конвертирующие оценку `coverConst` из натуральных чисел в вещественные.
  *Статус:* ранее существовавшая лемма `Circuit.powFamily_cover` была удалена; предстоит заново собрать применение энтропийного результата и извлечь покрытие.
...
- [ ] Заключить, что $|Rset| \le 2^{N - N^\delta}$ для $N = 2^n$ и всех $n \ge n_0$, и оформить итог в виде отдельной леммы, которую затем можно использовать при формулировке Леммы B-5.
  *Статус:* числовой модуль `Circuit.Numeric` и связанные оценки отсутствуют; необходимо восстановить цепочку неравенств, завершающуюся оценкой двойной экспоненты.

2.3. Ограничение покрытия конкретной функции
- [ ] Для конкретной функции $f$ определить $Rset_f = \{ R \in Rset \mid \exists x \in R: f(x) = 1 \}$, реализовав это как `Rset.filter` или `Finset.filter` в Lean.
- [ ] Доказать, что $Rset_f$ наследует монохроматичность и покрытие всех единичных точек $f$. Для этого нужно аккуратно переписать существующие определения (`AllOnesCovered`, `monoOn`) и убедиться, что фильтрация не нарушает условия на остальные функции семейства.
- [ ] Установить, что $|Rset_f| \le |Rset|$, сохраняя требуемую оценку на мощность, и оформить отдельной вспомогательной леммой (например, `filtered_cover_card_le`).
  *Статус:* реализация фильтрации покрытия была удалена; требуется заново построить переход от семейного покрытия к покрытию одной функции.
