...
lean_lib PnP3 where
  ...
  globs := #[
    ...
    Glob.one `Complexity.Uniform.V1.FixedGammaPayloadZeroCleanup,
    ...
    -- inherited (H14 since taken by G3a below).  Its malformed branch is the first routed reject
    -- the chain exercises.  Three
    ...
    -- Part A G3a: G2p-c's second-payload table composed with the whole G2z composite, one
    -- 68-state table whose single live routed row qScanLeft-on-blank targets G2z's start -- the
    -- newly executed handoff H14, taken at G2p-c's first arrival exactClock N zeros; H15, H16 and
    -- H17 are inherited.  The switch is also stated at the two degenerate decoded widths, which
    -- no accepted parsed target reaches.  Four handoffs of seventeen; the start still retags the
    ...
    -- G2p-a's scratch-bootstrap table and that whole composite as one closed 95-state, 285-row
    -- table.  H12 is the newly executed routed row `qScanLeft`-on-blank into index 9; H13, H14,
    -- H15, H16 and H17 are inherited.  One statement covers every decoded width, width zero
    -- included, at first arrival `2N - 11 - zeros`, and needs no room premise.  Six handoffs of
    -- seventeen; the start still retags the actual G2m dispatcher endpoint and the lane stays
    -- unfenced.  No raw-input execution, first arrival of the composed accept, or language
    -- acceptance is claimed.
    Glob.one
    ...
    Glob.one `Complexity.Uniform.V1.CombinedMachine,
    -- Total combined parser/verifier correctness and relation packaging.
    Glob.one `Complexity.Uniform.V1.CombinedCorrectness,
...
lean_lib Pnp4 where
  ...
  globs := #[
    ...
    Glob.one `Pnp4.Frontier.ContractExpansion.FiniteNandSameThresholdNoGo,
    ...
    Glob.one `Pnp4.Frontier.ContractExpansion.ContentCappedSizes,
    ...
    -- Part A: tagged V1 pair framing to headerless content semantics.
    Glob.one `Pnp4.Frontier.ContractExpansion.ThresholdTaggedContentFraming,
    ...
    -- of seventeen (H14 since taken by G3a below); the fourteen earlier handoffs, the fence, the
    -- witness checks, `AcceptsAt` and `ContentVerifierBridge` stay open.
    Glob.one
      `Pnp4.Frontier.ContractExpansion.ContentFixedGammaTargetMarkersLoopDecrementCountdownBridge,
    -- Part A G3a: the executed second-payload -> G2z handoff on the parsed target at the same
    -- cubic budget, listed after the G2z bridge, which it imports: one 68-state machine runs
    -- under exactly the same three hypotheses, switches blocks at G2p-c's first arrival
    -- `2 * N - 7` and reaches its accept at
    -- `(2N - 7) + exactClock + totalClock + composedClock <= B`.  Four handoffs of seventeen; the
    -- thirteen earlier handoffs, the fence, the witness checks, `AcceptsAt` and
    -- `ContentVerifierBridge` stay open.
    Glob.one
    ...
    -- Part A G3e: the same chained run one block further left again, at the same three hypotheses
    -- and the same cubic budget, through the newly executed H12.  The clock bound expands to
    -- `(2N - 11 - zeros) + (2N + zeros - 6) + (2N - 7) + exactClock + totalClock
    -- + composedClock <= B`.  Six handoffs of seventeen; the eleven earlier handoffs, the fence,
    -- the witness checks, `AcceptsAt` and `ContentVerifierBridge` stay open.
    Glob.one
