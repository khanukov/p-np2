...
lemma maxDepthIndexList_spec
  ...
  | x :: xs, i, h => by
      ...
      | none =>
          ...
          | tail hj' =>
              -- Противоречие: хвост пуст в смысле `none`, значит `xs = []`.
              -- Это следует из `maxDepthIndexList` для непустого списка.
              cases xs with
              | nil =>
                  simp at hj'
              | cons _ _ =>
                  simp [maxDepthIndexList] at hrest'
      | some j =>
...
lemma maxDepthIndex?_none_iff_length_zero
  ...
  · intro hnone
    by_contra hpos
    have hmem : (List.finRange F.length).length > 0 := by
      simpa using List.length_finRange_pos.mpr (Nat.pos_of_ne_zero hpos)
    cases hlist : List.finRange F.length with
    | nil =>
        simp at hmem
    | cons i xs =>
        simp [maxDepthIndex?, maxDepthIndexList, hlist] at hnone
  · intro hlen
    simp [maxDepthIndex?, maxDepthIndexList, hlen]
...
lemma badRestrictions_eq_canonicalCCDT_badFamilyDet
  ...
  simp [mem_badRestrictions, badEvent_canonicalCCDT_iff_badFamilyDet (F := F) (ρ := ρ) ht]
