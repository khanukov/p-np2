...
before the mark, after it, and after the cursor restore.
...
is chained, no run here starts from `G1M.initialConfig`, no invariant is stated
and no arbitrary operand-2 index is read.
-/
