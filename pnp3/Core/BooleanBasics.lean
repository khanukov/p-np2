...
/-
pnp3/Core/BooleanBasics.lean — базовые определения для SAL (булевы векторы,
подкубы и операторы `memB`/`errU`).  Ниже закрепляем «историческую» выжимку о
прогрессе и оставшихся шагах, чтобы передача задачи новому инженеру проходила
быстрее.

## Что уже сделано и где искать код

* **Сырые объекты SAL.** Определения `BitVec`, `Subcube`, операции
  `Subcube.assign`/`assignMany`, индикаторы `memB` и вспомогательные леммы живут
  прямо в этом файле.  Слой стабилен и используется всеми модулями SAL.

* **Axis/Tails-инфраструктура.** Файл `ThirdPartyFacts/MultiSwitching/Core.lean`
  содержит конструкции осей и хвостов (`AxisWitness`, `CombinedAxisWitness`,
  `TailAssignmentBundle` и пр.).  Там же реализована рекурсия
  `TailAssignmentBundle.popHead`/`toPDT`, позволяющая строить хвостовые PDT по
  спискам присваиваний.

* **Пулы селекторов.** Агрегация селекторов разных пакетов, finset-представления
  и свидетельства принадлежности находятся в
  `ThirdPartyFacts/MultiSwitching/SelectorPool.lean`.  Важные структуры:
  `LeafSelectorWitness`, `leafSelectorTailBundles` и вспомогательные списки
  присваиваний для будущих хвостов.

* **Документация по статусу.** В `ThirdPartyFacts/MultiSwitching_Gaps.md`
  описывается актуальный прогресс, перечисляются закрытые подпункты плана и
  список незакрытых `sorry`.  Этот файл нужно обновлять при каждом существенном
  шаге.

## Что осталось сделать (микрозадачи)

1. **Нормализация хвостов.** На основе `leafSelectorTailBundles` построить для
   каждого листа оси `β` хвостовой PDT `globalTail β`, у которого листья
   совпадают с нормализованными селекторами.  Каркас рекурсии уже реализован в
   `TailAssignmentBundle.toPDT`, осталось доказать дизъюнктность/покрытие
   списков и оценку глубины через длину хвоста.

2. **Заполнить `CombinedTailCertificate.tails`.** В модуле
   `ThirdPartyFacts/MultiSwitching/Core.lean` требуется построить поле `tails` и
   доказать `selectors_mem_global` без `sorry`, используя готовые леммы о
   поддержке и равенства `Subcube.assignMany`.

3. **Индуктивный шаг `Depth1WitnessInput.build`.** После появления глобальных
   хвостов реализовать `build`, упаковать хвосты в
   `AxisTailSystem.TailCertificate`, проверить бюджет `depthBudget M (d + 1)` и
   собрать ошибку `totalBadBound`.

4. **Удалить аксиому.** С готовым `build` заменить `partial_shrinkage_for_AC0`
   в `Facts/Switching/Facts_Switching.lean` на теорему и обновить интерфейсы
   `AC0PartialWitness`/smoke-тесты.

Эту памятку можно расширять по мере прогресса, чтобы новая команда быстрее
ориентировалась в исходниках.
-/
...
когда нужно последовательно применять несколько присваиваний и отслеживать
получающийся подкуб.
-/
@[simp] lemma assign_of_none {n : Nat} {β : Subcube n}
  {i : Fin n} {b : Bool} (hfree : β i = none) :
...
выполняем список `updates₁`, получая (возможно) новый подкуб, а затем
продолжаем обработку хвоста `updates₂`.  Формула удобно упакована через
монадический `bind`, что позволяет сразу расклеивать существование
промежуточного подкуба в леммах по индукции.
-/
@[simp] lemma assignMany_append {n : Nat} (β : Subcube n)
  (updates₁ updates₂ : List (BitFix n)) :
  Subcube.assignMany β (updates₁ ++ updates₂)
    = (Subcube.assignMany β updates₁).bind
        (fun β' => Subcube.assignMany β' updates₂) := by
induction updates₁ generalizing β with
| nil =>
    simp [Subcube.assignMany]
| cons pair rest ih =>
    rcases pair with ⟨i, b⟩
    simp [Subcube.assignMany, List.cons_append, ih]
