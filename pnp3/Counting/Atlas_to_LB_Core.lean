...
theorem unionClass_card_bound
  ...
  have hsurj : Function.Surjective toUnion := by
    ...
    have hg_eq : g.1 x = coveredB S x := by simp [hcover]
  ...
  have hdomain_card :
    ...
    calc
      ...
      _ = unionBound (R.toFinset.card) k := by simp [hdict_card]
...
by
  ...
  have h_inj : Function.Injective toSubset := by
    ...
    have hf₂ := flipOn_mismatchSet (m := m) (g := g) (f := f₂.1)
    calc
      f₁.1 = flipOn g (mismatchSet (m := m) g f₁.1) := by simp [hf₁]
      _ = flipOn g (mismatchSet (m := m) g f₂.1) := by simp [hsets]
      _ = f₂.1 := by simp [hf₂]
  have hcard_le :
      Fintype.card (HammingBallSubtype (m := m) (g := g) (ε := ε)) ≤
