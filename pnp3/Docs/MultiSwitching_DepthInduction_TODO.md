...
**Технический блокер:** в текущей абстракции `AC0Circuit` нет
канонической операции “снять верхний слой” (получить семейство подформул
меньшей глубины). Без такой операции индукция по глубине не может быть
формализована на уровне `FamilyIsAC0`.
...
6. **Декомпозиция AC0Circuit по слоям**:
   - Нужен ли явный API в `AC0/Formulas.lean` для получения семейства
     depth‑(d-1) подформул из depth‑d схемы, чтобы индукция была
