...
**Прогресс (обновлённо):** ~82%. Формульная часть закрыта, логические мосты и
пакет параметров готовы, добавлены леммы покрытия для вывода контрпримера при
`|Y|` выше ёмкости любого малого решателя. Недостаёт интеграции античекера и
балансировки в общую версию `OPS_trigger_general`.

**Вопросы, которые ускорят финализацию:**

1. Можно ли принять «как есть» асимптотические оценки античекера (ёмкость
   покрытия `m = polylog(N) · N^{1+α}` и мощность `|Y| ≳ 2^{N^{1+α}}`), или
   нужно формализовать точные коэффициенты? Это позволит сразу связать новые
   леммы покрытия с античекером.
2. Достаточно ли фиксированных параметров (ε = 1/10, α = 1/2, β = 3), или
   стоит оставить их символическими в финальном доказательстве? Жёсткая фиксация
   упростит расчёты, но может потребовать отдельной леммы для «любого ε > 0».
3. При балансировке схем достаточно сослаться на имеющиеся аксиомы
   `shrinkage_for_localCircuit` / `partial_shrinkage_for_AC0`, или нужен мост к
   конкретной Spira/Håstad-форме (например, глубина O(log N))? Это влияет на
   то, какая точная ёмкость обложек понадобится в шаге сравнения мощностей.
4. Требуется ли интегрировать античекер только для AC⁰-формул, а затем
   поднимать к общим схемам через уже имеющиеся мосты, или нужно сразу работать
   с оболочкой `SmallGeneralCircuitSolver`? Первый вариант проще, второй —
   короче по числу переходов.

### Текущее состояние D1

* Прогресс: ~80%. Формульная ветка полностью без аксиом, есть логический мост
  от `NP ⊆ P/poly` к наличию маленького решателя, подготовлен набор
  параметров по умолчанию и комбинаторные леммы для античекера.
