# OPS_trigger_general: формулировка, контекст и исторический план снятия аксиомы
...
на которой основаны гипотезы нижних границ. Исторически триггер был оформлен
как аксиома; теперь в `Facts_Magnification.lean` реализовано полное доказательство.
Ниже остаётся обзор исторического плана и Lean-ориентированных шагов, которые
привели к снятию аксиомы. Эти разделы сохранены как архив и не являются
актуальным планом работ.

## 1. Что утверждает `OPS_trigger_general`

**Декларация (теперь теорема).** В файле `Facts_Magnification.lean` триггер
реализован как доказательство
```lean
...
```
  ...
  - **Использование.** Мост `bridge_from_general_statement` и его производные
    ...
    теорема выступает финальным рычагом шага D, превращающим установленные нижние
    оценки в несодержание NP в P/poly.
...
- **Интерфейсы классов сложности.** Файл `Complexity/Interfaces.lean`
  предоставляет предикаты `Ppoly`, `NP` (через полиномиальный верификатор) и
  утверждения `NP_not_subset_Ppoly`, `P_subset_Ppoly_proof`, `P_ne_NP`. Они
  используются на шаге D для перевода несодержания NP в разделение P и NP.
  Дополнительно в `Models/Model_GapMCSP.lean` введены прямые леммы
  `gapMCSP_yes_of_small` и `gapMCSP_no_of_large`, которые переводят условия про
  малые/большие схемы в принадлежность YES/NO promise-областям и избавляют от
  ручного раскрытия `GapMCSPPromise`.
...
- Все мосты шага D (например, `bridge_from_LB_Formulas`) замыкают доказательство
  через `OPS_trigger_general` или его формульную специализацию. Теперь триггер
  реализован как теорема, и пайплайн использует его без внешних допущений.

## 4. Исторический план снятия аксиомы (математический эскиз, архив)

> **Примечание.** Раздел 4 сохраняется в исторических целях: это план, который
> использовался при формализации и уже выполнен.
...
## 6. Оставшиеся уточнения для снятия аксиомы D.1 (архив)
...
- **Строгое включение GapMCSP в NP.** Сейчас `NP` формализован через
  полиномиальный верификатор, а GapMCSP рассматривается как promise-задача.
  Нужен источник (или план формализации), как аккуратно вложить обещанную
  задачу в обычный язык и обосновать, что из предположения `NP ⊆ P/poly`
  следует наличие маленьких схем именно для выбранного `p : GapMCSPParams`.
- **Балансировка схем до AC⁰/локальных форм.** В Lean пока нет явной леммы
  ...
  shrinkage-факты (теоремы `partial_shrinkage_for_AC0` и
  `shrinkage_for_localCircuit` с внешним witness) построить неглубокую
  схему/формулу из произвольной полиномиальной схемы с контролем роста размера.
...
1. **GapMCSP ∈ NP.** Допустимо опираться на внешний источник (OPS’20 или CJW’22)
   ...
   При необходимости лемму можно переформулировать через стандартную
...
2. **Балансировка схем.** Разрешено использовать внешнюю теорему (вариант
   Спиры/Хåстада) или напрямую ссылаться на уже имеющиеся shrinkage-факты
   (`partial_shrinkage_for_AC0` и `shrinkage_for_localCircuit` как теоремы с
   внешним witness) как «чёрный ящик» для превращения полиномиальной
   схемы в неглубокую форму с контролируемым ростом размера. Дополнительное
   доказательство балансировки внутри pnp3 не требуется.
...
## 7. Финализация открытых вопросов и рабочий план доказательства (архив)
...
- **GapMCSP в NP (promise):** используем внешний источник OPS/CJW как готовое
  ...
  без SAT-редукции. Лемму можно переписать через сертификат «малая схема» при
  необходимости усилить связь с promise-формулировкой.
- **Балансировка:** опираемся на уже имеющиеся shrinkage-факты
  (`partial_shrinkage_for_AC0` и `shrinkage_for_localCircuit` как теоремы с
  внешним witness), либо на любой внешний вариант Спиры, как чёрный
  ящик, не доказывая балансировку с нуля.
...
2. **Балансировка:** к полученной схеме применяем shrinkage-факты
   `shrinkage_for_localCircuit` (теорема с внешним witness) и
   `partial_shrinkage_for_AC0` (теорема с внешним witness), получая неглубокий решатель (формулу
   AC⁰ или локальную схему) размера `poly(N)`; при необходимости упрощаем
   асимптотику до `N^{2+ε}`/`N^{1+ε}`.
...
- Лемма `OPS_trigger_formulas_contra` выводит противоречие с предположением
  `NP ⊆ P/poly` прямо из `FormulaLowerBoundHypothesis p δ`, переводя формульную
  гипотезу в общий формат и применяя `OPS_trigger_general_contra`.
- Соответственно `OPS_trigger_formulas` теперь получает `NP \nsubseteq P/poly`
  через `NP_not_subset_Ppoly_of_contra` без обращения к аксиоме D.1, а мосты
  `bridge_from_pipeline_general` и `bridge_from_pipeline_kit_general`
  используют эту конструктивную версию.
- Отдельно отмечаем, что теперь «малые решатели» описываются не только набором
  параметров, но и парой `(decide, correct)` для promise-версии GapMCSP. Это
  снимает необходимость вводить фиктивные «дефолтные» решатели и делает
  контрапозиции явными на уровне типов.
...
на гипотезу шага C и конструктивный контрапозитив OPS.
...
* Добавлена функция `coverWitness_from_antiChecker`, превращающая вывод
  ...
  теоремы античекера без ручной распаковки `let`-связок и приведения типов.
...
* Добавлена функция `coverWitness_from_antiChecker_testset` и её нормализованный
  ...
  ёмкость `m = unionBound * 2^{|T|}` из теоремы `antiChecker_exists_testset`
...
## 10. Аудит текущего состояния и «стоп-кадр» перед продолжением

Ниже — зафиксированный снимок состояния, чтобы позже было проще возобновить
работу над снятием D1.

- **Сборка/тесты:** `lake build` и `lake test` проходят на текущей ветке; весь
  пайплайн Lean компилируется. Это подтверждает целостность кода и отсутствие
  расхождений после последних чисток.
- **Где искать ключевые конструкции:**
  - Комбинаторные леммы покрытия и упаковки античекера —
    `pnp3/Magnification/Facts_Magnification.lean` (разделы с
    `CoverCapacityWitness`, `coverWitness_from_antiChecker_*`,
    `uncovered_from_antiChecker_*`, `AntiCheckerTestsetPackage`).
  - Мосты шага D, вызывающие триггеры —
    `pnp3/Magnification/Bridge_to_Magnification.lean`.
  - Описание оставшегося плана — этот файл (раздел 7.2) с конкретными пунктами
    2–6, которые ещё предстоит формализовать.
- **Что делать дальше (кратко):**
  1. Взять решатель из предположения `NP ⊆ P/poly` (лемма уже есть в
     `Facts_Magnification.lean`) и прогнать через балансировку (теоремы
     `partial_shrinkage_for_AC0` и `shrinkage_for_localCircuit` с внешним
     witness), чтобы получить малую формулу/схему.
  2. Применить античекер к этой формуле и сразу использовать готовый пакет
     `antiChecker_testset_package_inputLen`, чтобы получить свидетельство о
     неполном покрытии `Y`.
  3. Сравнить мощность `Y` с числом малых схем (можно через грубые оценки
     `2^{O(N^{1+α} \log N)}`) и извлечь явного «запрещённого» решателя, дающего
     `False` в гипотезе шага C.
  4. Обновить `OPS_trigger_general`, убрав аксиому и подключив конструктивный
     аргумент. После этого мосты шага D будут полностью аксиомно-свободны.
- **Вопросов на сейчас нет:** все уточнения собраны выше; следующий шаг —
  реализация пункта 7.2(2–6) в Lean.
