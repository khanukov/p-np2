...
- **Интерфейсы классов сложности.** Файл `Complexity/Interfaces.lean`
  предоставляет предикаты `Ppoly`, `NP` (определён через полиномиальный
  верификатор) и утверждения `NP_not_subset_Ppoly`, `P_subset_Ppoly_proof`,
  `P_ne_NP`. Они используются на шаге D для перевода несодержания NP в
  разделение P и NP.
...
- **Строгое включение GapMCSP в NP.** `NP` определён через полиномиальный
  верификатор, а GapMCSP рассматривается как promise-задача. Нужен источник
  (или план формализации), как аккуратно вложить обещанную задачу в обычный
  язык и обосновать, что из предположения `NP ⊆ P/poly` следует наличие
  маленьких схем именно для выбранного `p : GapMCSPParams`.
...
1. **GapMCSP ∈ NP.** Допустимо опираться на внешний источник (OPS’20 или CJW’22)
   ...
   при детализации модели её можно переписать через стандартную верификацию
   сертификата (малая схема как свидетельство для Yes-случая).
...
- **GapMCSP в NP (promise):** используем внешний источник OPS/CJW как готовое
  ...
  без SAT-редукции. При уточнении модели лемма переписывается через сертификат
  «малая схема».
