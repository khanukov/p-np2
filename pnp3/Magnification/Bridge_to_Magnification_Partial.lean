...
theorem NP_not_subset_Ppoly_from_partial_formulas
  ...
  (hLocalized : LowerBounds.LocalizedFamilyWitnessHypothesis_partial p) :
  ...
  exact OPS_trigger_formulas_partial (p := p) (δ := δ) hLocalized hHyp

theorem P_ne_NP_from_partial_formulas
  {p : GapPartialMCSPParams} {δ : Rat} (hδ : (0 : Rat) < δ)
  (hLocalized : LowerBounds.LocalizedFamilyWitnessHypothesis_partial p) : P_ne_NP := by
  have hNP : NP_not_subset_Ppoly :=
    NP_not_subset_Ppoly_from_partial_formulas (p := p) (δ := δ) hδ hLocalized
