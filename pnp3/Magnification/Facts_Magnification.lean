...
/--
  Расширенный пакет данных античекера с тестовым множеством.  В отличие от
  `CoverCapacityWitness` он хранит не только `Y` и ёмкость покрытия `m`, но и
  сам сценарий `sc`, тестовое множество `T`, а также факт принадлежности
  `Y ⊆ ApproxOnTestset (sc, T)`.  Такой пакет удобен для применения
  комбинаторных лемм о покрытиях и оценки `approxOnTestset_card_le` из части B
  без ручных приведений типов и переписывания `let`-связок античекера.
-/
structure AntiCheckerTestsetPackage (n : Nat) : Type where
  sc : BoundedAtlasScenario n
  T : Finset (Core.BitVec n)
  Y : Finset (Core.BitVec n → Bool)
  hSubset : Y ⊆ familyFinset (sc := sc)
  hApprox : ∀ f ∈ Y,
    f ∈ Counting.ApproxOnTestset (R := sc.atlas.dict) (k := sc.k) (T := T)
  hT : T.card ≤ Models.polylogBudget n
  hCapacity :
    Counting.unionBound (Counting.dictLen sc.atlas.dict) sc.k * 2 ^ T.card <
      Y.card

/--
  Естественный пакет ёмкости, производимый из расширенного пакета античекера.
-/
noncomputable def AntiCheckerTestsetPackage.coverWitness
    {n : Nat} (pkg : AntiCheckerTestsetPackage n) :
    CoverCapacityWitness (Core.BitVec n → Bool) :=
  { Y := pkg.Y
    m := Counting.unionBound (Counting.dictLen pkg.sc.atlas.dict) pkg.sc.k *
        2 ^ pkg.T.card
    hLarge := pkg.hCapacity }

/--
  Количественная оценка мощности множества `ApproxOnTestset` из пакета
  античекера: число функций, совпадающих с некоторым объединением подкубов вне
  `T`, ограничено величиной `m` из `coverWitness`.
-/
lemma AntiCheckerTestsetPackage.approx_card_le
    {n : Nat} (pkg : AntiCheckerTestsetPackage n) :
    Fintype.card
        (Counting.ApproxOnTestsetSubtype
          (R := pkg.sc.atlas.dict) (k := pkg.sc.k) (T := pkg.T))
      ≤ pkg.coverWitness.m := by
  classical
  -- Кардинальная оценка напрямую следует из `approxOnTestset_card_le`.
  simpa [AntiCheckerTestsetPackage.coverWitness]
    using (Counting.approxOnTestset_card_le (R := pkg.sc.atlas.dict)
      (k := pkg.sc.k) (T := pkg.T))

/--
  Прямое извлечение пакета античекера (с тестовым множеством) на длине
  `solver.ac0.n`.  Включает сценарий `sc`, тестовое множество `T`, семейство
  `Y`, принадлежность `Y` обоим ограничениям античекера и ёмкостную оценку
  `unionBound * 2^{|T|} < |Y|`.
-/
noncomputable def antiChecker_testset_package
    {p : GapMCSPParams} (solver : SmallAC0Solver p) :
    AntiCheckerTestsetPackage solver.ac0.n := by
  classical
  -- Распаковываем усиленную аксиому античекера с тестовым множеством.
  let hExist := antiChecker_exists_testset (p := p) solver
  let F : Family (Models.inputLen p) := Classical.choose hExist
  ...
  let T : Finset (Core.BitVec (Models.inputLen p)) := Classical.choose hExistT
  let hProps := Classical.choose_spec hExistT
  -- Приводим все объекты к длине `solver.ac0.n` один раз.
  set Fsolver : Family solver.ac0.n := solver.same_n.symm ▸ F
  set scWitness : BoundedAtlasScenario solver.ac0.n :=
    (scenarioFromAC0 (params := solver.ac0) Fsolver).2
  set Ysolver : Finset (Core.BitVec solver.ac0.n → Bool) := solver.same_n.symm ▸ Y
  set Tsolver : Finset (Core.BitVec solver.ac0.n) := solver.same_n.symm ▸ T
  -- Разворачиваем свойства аксиомы без тяжёлых упрощений.
  dsimp [F, Y, T, Fsolver, scWitness, Ysolver, Tsolver] at hProps
  rcases hProps with ⟨hSubset, _hCap, hT, hApprox, hUnion⟩
  -- Переписываем принадлежность к `ApproxOnTestset` в нормализованном виде.
  have hApprox' : ∀ f ∈ Ysolver,
      f ∈ Counting.ApproxOnTestset (R := scWitness.atlas.dict)
        (k := scWitness.k) (T := Tsolver) := by
    intro f hf
    have h := hApprox f hf
    simpa [F, Y, T, Fsolver, scWitness, Ysolver, Tsolver] using h
  -- Собираем пакет на длине `solver.ac0.n`.
  refine
    { sc := scWitness
      T := Tsolver
      Y := Ysolver
...
/--
  Вариант пакета античекера, нормализованный к канонической длине `inputLen p`.
  Устраняет необходимость вручную применять равенство `same_n` в дальнейших
  шагах триггера.
-/
noncomputable def antiChecker_testset_package_inputLen
    {p : GapMCSPParams} (solver : SmallAC0Solver p) :
    AntiCheckerTestsetPackage (Models.inputLen p) := by
  classical
  simpa using
    (solver.same_n ▸ antiChecker_testset_package (p := p) solver)
...
/--
  Усиленное противоречие: из пакета античекера с тестовым множеством следует,
  что существует покрытие мощностью `m`, равной `unionBound * 2^{|T|}`.  Тогда
  универсальный Finset всех функций из `ApproxOnTestset` уже обладает
  кардинальностью не больше `m`, поэтому лемма `witness_not_exists_full_cover`
  приводит к противоречию с включением `Y ⊆` этого покрытия.  Таким образом,
  сам по себе античекер обеспечивает `False` для любого предполагаемого малого
  решателя.
-/
theorem antiChecker_testset_package_contradiction
    {p : GapMCSPParams} (solver : SmallAC0Solver p) : False := by
  classical
  -- Извлекаем нормализованный пакет античекера.
  let pkg := antiChecker_testset_package_inputLen (p := p) solver
  -- Покрытие: все функции, совпадающие с объединением подкубов вне T.
  let cover : Unit → Finset (Core.BitVec (Models.inputLen p) → Bool) := fun _ =>
    (Finset.univ : Finset
      (Counting.ApproxOnTestsetSubtype
        (R := pkg.sc.atlas.dict) (k := pkg.sc.k) (T := pkg.T))).image Subtype.val
  have hCap : ∀ s, (cover s).card ≤ pkg.coverWitness.m := by
    intro _
    -- `card_image_le` даёт оценку через мощность всего подтипа, затем применяем
    -- `approx_card_le` из пакета.
    have hCardImage : (cover ()).card ≤
        Fintype.card
          (Counting.ApproxOnTestsetSubtype
            (R := pkg.sc.atlas.dict) (k := pkg.sc.k) (T := pkg.T)) := by
      -- `cover ()` равно `cover s` для любого `s`, так что выберем `()` для удобства.
      dsimp [cover]
      simpa using
        (Finset.card_image_le (s := (Finset.univ : Finset
          (Counting.ApproxOnTestsetSubtype (R := pkg.sc.atlas.dict)
            (k := pkg.sc.k) (T := pkg.T)))) (f := Subtype.val))
    exact hCardImage.trans pkg.approx_card_le
  -- Применяем ёмкостную лемму: семейство `Y` не может быть целиком покрыто.
  have hNoCover :=
    witness_not_exists_full_cover (w := pkg.coverWitness) cover hCap
  -- Но по условию античекера каждое `f ∈ Y` принадлежит `ApproxOnTestset`,
  -- значит лежит в `cover ()`. Получаем явное противоречие.
  have hSub : pkg.coverWitness.Y ⊆ cover () := by
    intro f hf
    -- Используем свидетельство принадлежности `ApproxOnTestset` из пакета.
    have hfApprox := pkg.hApprox f hf
    dsimp [cover]
    -- Построим элемент подтипа с тем же значением.
    refine Finset.mem_image.mpr ?_
    refine ⟨⟨f, hfApprox⟩, by simp, by simp⟩
  exact hNoCover ⟨(), hSub⟩
