...
/-!
  ...
  * аналогично для классов локальных схем (`N · polylog(N)`).
  ...
  Здесь они реализованы как теоремы, собранные в одном файле ради удобного
  использования в мостах шага D.  Триггеры опираются на уже зафиксированные
  факты нижних границ и, в частности, на аксиомы шага A из
  `ThirdPartyFacts/Facts_Switching.lean`; новых аксиом в этом файле не
  вводится.  Каждый триггер снабжён комментариями с отсылкой к источникам
  (OPS’20, JACM’22 и др.), чтобы ясно понимать математический контекст.
-/
...
lemma formula_hypothesis_default {p : GapMCSPParams}
    (h : AC0BoundedStatement p opsDefaultPack.ε) :
...
by
  ...
  exact OPS_trigger_general (p := p) (ε := ε) (statement := GeneralCircuitStatement p)
    hF_all hHyp
...
theorem OPS_trigger_general_circuits_default
  ...
  (hF_all : ∀ loc : SmallLocalCircuitSolver p,
    ThirdPartyFacts.FamilyIsLocalCircuit loc.params.params
      (Counting.allFunctionsFamily loc.params.params.n)) :
  GeneralCircuitStatement p → NP_not_subset_Ppoly :=
by
  intro hNoSolver
