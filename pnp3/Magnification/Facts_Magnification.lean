...
-/
def AntiCheckerTestsetPackage.coverWitness
...
noncomputable def antiChecker_testset_package
  ...
  classical
  -- Распаковываем аксиому античекера с тестовым множеством.
  let hExist := antiChecker_exists_testset (p := p) solver
  let F : Family (Models.inputLen p) := Classical.choose hExist
  have hExistY := Classical.choose_spec hExist
  let Y : Finset (Core.BitVec (Models.inputLen p) → Bool) := Classical.choose hExistY
  have hExistT := Classical.choose_spec hExistY
  let T : Finset (Core.BitVec (Models.inputLen p)) := Classical.choose hExistT
  have hProps := Classical.choose_spec hExistT
  -- Переносим всё на длину `solver.ac0.n` (через равенство `same_n`).
  let Fsolver : Family solver.ac0.n := solver.same_n.symm ▸ F
  let scWitness : BoundedAtlasScenario solver.ac0.n :=
    (scenarioFromAC0 (params := solver.ac0) Fsolver).2
  let Ysolver : Finset (Core.BitVec solver.ac0.n → Bool) := solver.same_n.symm ▸ Y
  let Tsolver : Finset (Core.BitVec solver.ac0.n) := solver.same_n.symm ▸ T
  -- Явно раскрываем свойства, удаляя `let`-связки.
  dsimp [Fsolver, scWitness, Ysolver, Tsolver] at hProps
...
theorem antiChecker_testset_package_contradiction
  ...
  have hCap : ∀ s, (cover s).card ≤ pkg.coverWitness.m := by
    ...
    have hCardImage : (cover ()).card ≤
      ...
      dsimp [cover]
      simpa using (Finset.card_image_le (Finset.univ : Finset
        (Counting.ApproxOnTestsetSubtype (R := pkg.sc.atlas.dict)
          (k := pkg.sc.k) (T := pkg.T))) Subtype.val)
