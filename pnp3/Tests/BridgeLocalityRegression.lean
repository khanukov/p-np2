...
theorem i4_gap_targeted_payload_contradiction_of_subcubeRestriction
    {p : GapPartialMCSPParams}
    (pkg : Pnp3.LowerBounds.AbstractGapTargetedSingletonDensityPayload p)
    ...
    (hStable :
      ∀ x : Core.BitVec (Models.partialInputLen p),
        pkg.base.f
            (castBitVec pkg.hsame.symm
              ((factsRestrictionOfSubcube β).apply x))
          =
        pkg.base.f (castBitVec pkg.hsame.symm x)) :
    False := by
  exact Pnp3.LowerBounds.false_of_abstractGapTargetedPayload_of_subcubeRestriction
