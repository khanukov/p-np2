...
otherwise.  `check_seq_literal_probe` reduces their composition `pair` by kernel computation: six
states, start `0`, accept `4`, reject `5`; on the empty word the composed control is the reader's
start `3` after **one** step — the writer's accepting step, routed, the zero-cost handoff made
visible — and the composed accept after two; on the one-bit word `true` the composed reject after
one.  Each conjunct is a claim about its own literal input only.
...
/-- The two embeddings replace the control and nothing else: the six projection equations. -/
theorem check_seqEmbed_pins (M₁ M₂ : UniformTM) {n B : Nat} (c : Config M₁.stateCount n B)
    (d : Config M₂.stateCount n B) :
...
the composed reject after one step, on cell `0`; step four shows it absorbing. -/
