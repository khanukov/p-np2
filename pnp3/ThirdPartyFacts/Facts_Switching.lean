/-
  ...
  как interface-level witness/provider assumptions (без глобальных `axiom`
  в активном дереве), и снабжены подробными комментариями, чтобы позднее было
  понятно, какие именно параметры предстоит доказать или построить внутри проекта.
