...
/-
  Конструктивное построение PDT, чьи листья содержат заданные подкубы.

  Этот блок используется как в depth‑2 shrinkage, так и в построении
  «слабого» shrinkage‑свидетеля, поэтому его нужно объявить до первых
  вызовов `buildPDTFromSubcubes`.
-/
/--
