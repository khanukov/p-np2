...
noncomputable def chooseCombinedTailCertificate
  ...
  · intro β hβ f hf γ hγ
    -- Искомое утверждение следует напрямую из свойства
    -- `selectors_mem_global`, зафиксированного в `CombinedTailCertificate`.
    exact
      C.selectors_mem_global (n := n) (M := M) (τ := τ) (w := w) (t := t)
        (packages := packages) (β := β) hβ (f := f) hf (γ := γ) hγ
...
noncomputable def CombinedTailCertificate.partialCertificate
  ...
  · -- принадлежность селекторов хвостам
    intro β hβ f γ hγ
    classical
    by_cases hf : f ∈ cnfFamily
        (Fs := flattenedCNFs (n := n) (M := M) (τ := τ)
          (w := w) (t := t) packages)
    · -- Если функция действительно принадлежит объединённому семейству,
      -- то нужное свойство «листовости» немедленно следует из поля
      -- `selectors_mem_global` комбинированного хвостового сертификата.
      have hγ' : γ ∈ combinedTailSelectors (n := n) (M := M) (τ := τ)
          (w := w) (t := t) packages C.witness.axis β hβ f := by
        simpa [hf]
      exact C.selectors_mem_global (n := n) (M := M) (τ := τ)
        (w := w) (t := t) (packages := packages) (β := β) hβ (f := f) hf
        (γ := γ) hγ'
    · -- В противном случае селекторы пусты, и предпосылка противоречива.
      have hfalse : False := by
        simpa [combinedTailSelectors, hf] using hγ
      exact hfalse.elim
...
noncomputable def CombinedTailCertificate.tailCertificate
  ...
  { level := τ
    ...
    selectors_mem := by
      intro f γ hγ
      classical
      -- Для удобства обозначаем частичный сертификат.
      set PC := CombinedTailCertificate.partialCertificate
          (n := n) (M := M) (τ := τ) (w := w) (t := t)
          (packages := packages) C with hPC
      -- Разбор по принадлежности функции объединённому семейству.
      by_cases hf : f ∈ cnfFamily
          (Fs := flattenedCNFs (n := n) (M := M) (τ := τ)
            (w := w) (t := t) packages)
      · -- В этом случае достаточно сослаться на свойство `selectors_sub`
        -- у построенного частичного сертификата.
        simpa [hPC] using PC.selectors_sub (f := f) (β := γ) hf hγ
      · -- Для внешних функций список селекторов пуст, поэтому
        -- предпосылка невозможна.
        have hnil : PC.selectors f = [] := by
          -- Раскрываем определение селекторов и используем `hf : False`.
          classical
          simpa [hPC, CombinedTailCertificate.partialCertificate_selectors,
            Axis.selectorsFromTails, combinedTailSelectors, hf]
        have hfalse : False := by simpa [hnil] using hγ
        exact hfalse.elim
    depth_le := by
