...
1. **Глобальные хвосты.** Для успешной сборки шага `d → d + 1` требуется явное
   ...
   конструкции, которая собирает такие хвосты из данных пакетов.  При этом
   `combinedTailSelectors` для каждой функции берёт локальные селекторы ровно из
   того пакета, где лежит исходная формула.  Чтобы перенести их на общий хвост,
   нужно чётко понимать, **как именно объединяются хвостовые PDT разных
   пакетов**: совпадают ли они после нормализации, достаточно ли выбрать один
   фиксированный пакет, или необходимо построить новое дерево с объединённым
   набором листьев.  Без такой спецификации невозможно доказать требуемое
   свойство `selectors_mem_global`.
