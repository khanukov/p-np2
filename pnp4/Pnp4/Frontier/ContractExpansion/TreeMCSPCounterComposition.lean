...
/-! ### Doubly-nested composition lift (`seqNested`): decrement as the head of an inner `seq`

For the flattened binary→unary loop body `binToUnaryBody` (`TM_VERIFIER_STRATEGY.md` §12, D2t-3c-γ),
`selfLoopDecrement` is the **second** element of `seqList [stepRightOnce, selfLoopDecrement, …]`
`= seq stepRightOnce (seq selfLoopDecrement R)`, i.e. it sits in the doubly-nested `P2∘P1` position
`seq P1 (seq selfLoopDecrement R)` (`P1 = stepRightOnce`, `R` the chain tail).  These lemmas re-derive
the borrow ripple + stop + `counterValue` decrease in that position — the depth-2 rung of the assembly
ladder, the exact analogue of `gammaSelfLoopScan_seqNested_*`.  Standard axiom triple only. -/

/-- Nested borrow step (outer phase `P1.numPhases`, inner decrement phase `0`, bit `0`): phase stays. -/
...
/-- Nested stop step (bit `1`): the phase becomes the inner shifted accept phase `P1.numPhases + 1`. -/
...
/-- Borrow-ripple as a doubly-nested phase, from an arbitrary start `c0` (outer phase `P1.numPhases`):
if the window `[c0.head, c0.head + k)` is all `0`, then after `k` steps the phase still rests at
`P1.numPhases`, the head has advanced to `c0.head + k`, and exactly those `k` cells are set to `1`.
Depth-2 analogue of `selfLoopDecrement_seqP2_runConfig_borrow`. -/
theorem selfLoopDecrement_seqNested_runConfig_borrow
    (P1 R : ConstStatePhasedProgram Unit) {L : Nat}
  ...
  | succ k ih =>
      ...
      have hbnd : (c.head : Nat) + 1
        ...
        have htl : L + 2 ≤ (seq P1 (seq selfLoopDecrement R)).toPhased.toTM.tapeLength L := by
          show L + 2 ≤ L + (P1.timeBound L + (seq selfLoopDecrement R).timeBound L + 1) + 1
          omega
        rw [hhd]; omega
      have hbit : c.tape c.head = false := by
        rw [htp]
...
/-- After-decrement configuration as a doubly-nested phase: if the window `[c0.head, c0.head + j)` is
all `0` and cell `c0.head + j` is `1`, then after `j + 1` steps the phase is `P1.numPhases + 1`, head
`c0.head + j`, cells `[c0.head, c0.head + j)` set and cell `c0.head + j` cleared.  Depth-2 analogue of
`selfLoopDecrement_seqP2_runConfig_stop`. -/
theorem selfLoopDecrement_seqNested_runConfig_stop
    (P1 R : ConstStatePhasedProgram Unit) {L : Nat}
    (c0 : Configuration (M := (seq P1 (seq selfLoopDecrement R)).toPhased.toTM) L)
    (hphase : (c0.state.fst : Nat) = P1.numPhases) (j : Nat) (hj : (c0.head : Nat) + j ≤ L)
    (h_zeros : ∀ p : Fin ((seq P1 (seq selfLoopDecrement R)).toPhased.toTM.tapeLength L),
      (c0.head : Nat) ≤ (p : Nat) → (p : Nat) < (c0.head : Nat) + j → c0.tape p = false)
    (h_one : ∀ hb : (c0.head : Nat) + j
        < (seq P1 (seq selfLoopDecrement R)).toPhased.toTM.tapeLength L,
      c0.tape ⟨(c0.head : Nat) + j, hb⟩ = true) :
    (((TM.runConfig (M := (seq P1 (seq selfLoopDecrement R)).toPhased.toTM) c0 (j + 1)).state).fst
        : Nat) = P1.numPhases + 1
  ...
  obtain ⟨hph, hhd, htp⟩ :=
    selfLoopDecrement_seqNested_runConfig_borrow P1 R c0 hphase j hj h_zeros
  rw [TM.runConfig_succ]
...
/-- Decrement `counterValue` (`before = after + 1`) survives the doubly-nested composition: on
`seq P1 (seq selfLoopDecrement R)` from `c0` (outer phase `P1.numPhases`), if the window
`[c0.head, c0.head + k)` has first-one at offset `j` (`j < k`, `c0.head + k ≤ L`), then after `j + 1`
steps the little-endian value over that window has decreased by exactly one.  Depth-2 analogue of
`selfLoopDecrement_seqP2_runConfig_counterValue` — the headline rung for γ's element-2. -/
theorem selfLoopDecrement_seqNested_runConfig_counterValue
    (P1 R : ConstStatePhasedProgram Unit) {L : Nat}
    (c0 : Configuration (M := (seq P1 (seq selfLoopDecrement R)).toPhased.toTM) L)
    (hphase : (c0.state.fst : Nat) = P1.numPhases) (j k : Nat) (hjk : j < k)
    (hk : (c0.head : Nat) + k ≤ L)
  ...
  refine counterValue_first_one_diff c0
    ...
    (c0.head : Nat) j k hjk (by
      show (c0.head : Nat) + k ≤ L + (P1.timeBound L + (seq selfLoopDecrement R).timeBound L + 1) + 1
      omega) ?_ ?_ ?_ ?_ ?_
  · intro i hij hb
    exact h_zeros ⟨(c0.head : Nat) + i, hb⟩ (Nat.le_add_right _ _) (Nat.add_lt_add_left hij _)
  · intro hb
