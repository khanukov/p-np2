...
First layout brick of the NP-verifier track (see this directory's `README.md`).
...
zeros from the `z`-bit payload.  The planned read anchors symmetrically here. -/
...
/-- **Mirror-read position validity** (the payload read's core geometric precondition).  For a
