...
open Pnp4.Frontier.DagSupportCardinality

#check @wireDirectInputCover
#check @gateDirectInputCover
#check @directInputCover
#check @supportAt_subset_directInputCover
#check @support_subset_directInputCover
#check @directInputCover_card_le_two_mul_size
#check @support_card_le_two_mul_size
...
section ContentFixedGammaPayloadPendingSemanticBridgeSurface

open Pnp4.Frontier.ContractExpansion
open Pnp3.Complexity.Uniform.V1
open Pnp3.Complexity.Uniform.V1.PairEncoding

theorem check_VZR_allZeroSlice_eq_none_iff
    {N T offset width : Nat} (z : PrefixBitVec N) (hwidth : 0 < width) :
    VirtualZeroTailReader.allZeroSlice? z T offset width = none ↔
      T < offset + width :=
  VirtualZeroTailReader.allZeroSlice?_eq_none_iff z hwidth

theorem check_VZR_allZeroSlice_eq_some_false_iff
    {N T offset width : Nat} (z : PrefixBitVec N)
    (hfit : offset + width ≤ T) :
    VirtualZeroTailReader.allZeroSlice? z T offset width = some false ↔
      ∃ t, t < width ∧ padRead z (offset + t) = true :=
  VirtualZeroTailReader.allZeroSlice?_eq_some_false_iff z hfit

theorem check_true_cleanup_endpoint_and_allZeroSlice
    ...
    (htrue : FixedContentTagGate.physicalSymbol (Fin.append x w)
      (9 + zeros + k) = some true) (hfit : 9 + 2 * zeros ≤ T) :
    ...
    d.tape = FixedPairContentMarkerErase.contentTape B x w ∧
    VirtualZeroTailReader.allZeroSlice? (Fin.append x w) T
      (9 + zeros) zeros = some false :=
  true_cleanup_endpoint_and_allZeroSlice (B := B) x w htag hg hk hkz
    hprefix htrue hfit

theorem check_virtual_cleanup_endpoint_and_allZeroSlice
    {a m B zeros k T : Nat} (x : Bitstring a) (w : Bitstring m)
    (htag : FixedContentTagGate.tagMatches (Fin.append x w) = true)
    (hg : FixedContentGammaTerminator.gammaZeros? (Fin.append x w) = some zeros)
    (hk : 1 ≤ k) (hkz : k < zeros)
    (hprefix : ∀ j, 9 + zeros ≤ j → j < 9 + zeros + k →
      FixedContentTagGate.physicalSymbol (Fin.append x w) j = some false)
    (hvirtual : 9 + zeros + k = a + m) (hfit : 9 + 2 * zeros ≤ T) :
    let d := FixedGammaPayloadPendingCleanup.machine.run
      (FixedGammaPayloadPendingCleanup.cleanupClock zeros k)
      (FixedGammaPayloadPendingCleanup.startConfig B x w zeros k)
    ...
    d.tape = FixedPairContentMarkerErase.contentTape B x w ∧
    VirtualZeroTailReader.allZeroSlice? (Fin.append x w) T
      (9 + zeros) zeros = some true :=
  virtual_cleanup_endpoint_and_allZeroSlice (B := B) x w htag hg hk hkz
    hprefix hvirtual hfit

theorem check_virtual_allZeroSlice_at_physical_length
    {a m zeros k : Nat} (x : Bitstring a) (w : Bitstring m)
    (hk : 1 ≤ k) (hkz : k < zeros)
    (hvirtual : 9 + zeros + k = a + m) :
    VirtualZeroTailReader.allZeroSlice? (Fin.append x w) (a + m)
      (9 + zeros) zeros = none :=
  virtual_allZeroSlice_at_physical_length x w hk hkz hvirtual

theorem check_true_cleanup_endpoint_and_allZeroSlice_shared_window
  ...
  true_cleanup_endpoint_and_allZeroSlice_shared_window (B := B)
    x w htag hg hk hkz hprefix htrue

theorem check_virtual_cleanup_endpoint_and_allZeroSlice_shared_window
    {a m B zeros k : Nat} (x : Bitstring a) (w : Bitstring m)
    (htag : FixedContentTagGate.tagMatches (Fin.append x w) = true)
    (hg : FixedContentGammaTerminator.gammaZeros? (Fin.append x w) = some zeros)
    (hk : 1 ≤ k) (hkz : k < zeros)
  ...
  virtual_cleanup_endpoint_and_allZeroSlice_shared_window (B := B)
    x w htag hg hk hkz hprefix hvirtual

theorem check_cleanup_endpoint_and_allZeroSlice_shared_window
    {a m B zeros : Nat} (x : Bitstring a) (w : Bitstring m)
    (htag : FixedContentTagGate.tagMatches (Fin.append x w) = true)
    (hg : FixedContentGammaTerminator.gammaZeros? (Fin.append x w) = some zeros)
    (hzero : 0 < zeros)
    ...
    VirtualZeroTailReader.allZeroSlice? (Fin.append x w)
      (2 * (a + m) + 1) (9 + zeros) zeros = some true :=
  cleanup_endpoint_and_allZeroSlice_shared_window (B := B)
...
section ContentFixedGammaTargetSecondPayloadBridgeSurface

open Pnp3.Complexity.Uniform.V1
open Pnp4.Frontier.ContractExpansion

theorem check_header_digits {N n consumed : Nat} (z : PrefixBitVec N)
    ...
    ∃ zeros, FixedContentGammaTerminator.gammaZeros? z = some zeros ∧
      ...
      ∀ t, t < zeros →
        (FixedContentTagGate.physicalSymbol z (9 + zeros + t)).getD false =
          (n + 1).testBit (zeros - 1 - t) :=
  header_digits z hheader

theorem check_secondPayload_positive_register {a m B n consumed : Nat}
    (x : Bitstring a) (w : Bitstring m)
    (htag : FixedContentTagGate.tagMatches (Fin.append x w) = true)
    ...
    ∃ zeros, 2 ≤ zeros ∧ consumed = 2 * zeros + 1 ∧
      ...
      d.state = FixedGammaTargetSecondPayload.qDone ∧ d.head.val = 7 ∧
        ...
        d.tape ⟨a + m + 3, hroom⟩ = some ((n + 1).testBit (zeros - 2)) :=
  secondPayload_positive_register x w htag hheader hn hroom

theorem check_secondPayload_width_one_register {a m B n : Nat}
    (x : Bitstring a) (w : Bitstring m)
    (htag : FixedContentTagGate.tagMatches (Fin.append x w) = true)
    ...
    d.state = FixedGammaTargetSecondPayload.qDone ∧ d.head.val = 7 ∧
      ...
      ∀ i : Fin (tapeLength (PairEncoding.pairLength a m) B), a + m + 2 < i.val →
        d.tape i = none :=
  secondPayload_width_one_register x w htag hheader hroom

theorem check_secondPayload_zero_width_register {a m B : Nat}
    (x : Bitstring a) (w : Bitstring m)
    (htag : FixedContentTagGate.tagMatches (Fin.append x w) = true)
    (hheader : contentHeader? (Fin.append x w) = some (0, 1)) :
    let d := FixedGammaTargetSecondPayload.machine.run
    ...
    d.state = FixedGammaTargetSecondPayload.qDone ∧ d.head.val = 7 ∧
      ...
      ∀ i : Fin (tapeLength (PairEncoding.pairLength a m) B), a + m + 1 < i.val →
        d.tape i = none :=
  secondPayload_zero_width_register x w htag hheader

/-- Width two with both payload cells physically present: the gamma is `00 1`
and the payload window `[11, 13)` holds `11₂`, so the header is `(6, 5)`. -/
private def headerValuePhysicalPair : Bitstring 5 :=
  ![false, false, true, true, true]

example :
    FixedContentTagGate.tagMatches
        (Fin.append headerValueTag headerValuePhysicalPair) = true ∧
      FixedContentGammaTerminator.gammaZeros?
        (Fin.append headerValueTag headerValuePhysicalPair) = some 2 ∧
      contentHeader? (Fin.append headerValueTag headerValuePhysicalPair) =
        some (6, 5) := by
  decide

/-! On the G2o words (`a = 8`, so `2 ≤ a + B` and every budget allocates the
target cell `a + m + 3`), for every `B` and without running any machine.  Each
probe derives its cells from the bridge theorems above, so it pins what those
...
allocate, so the claim is not vacuous — stays blank: no third digit. -/
theorem probe_secondPayload_width_one (B : Nat) :
    let d := FixedGammaTargetSecondPayload.machine.run
      (FixedGammaTargetSecondPayload.deadline (8 + 3))
      (FixedGammaTargetSecondPayload.startConfig B headerValueTag headerValuePhysicalOne)
    d.state = FixedGammaTargetSecondPayload.qDone ∧
      d.tape ⟨8 + 3 + 1, by unfold tapeLength PairEncoding.pairLength; omega⟩ =
        some true ∧
      d.tape ⟨8 + 3 + 2, by unfold tapeLength PairEncoding.pairLength; omega⟩ =
        some true ∧
      d.tape ⟨8 + 3 + 3, by unfold tapeLength PairEncoding.pairLength; omega⟩ =
        none := by
  obtain ⟨hq, -, -, h1, h2, hblank⟩ :=
    secondPayload_width_one_register (B := B) headerValueTag headerValuePhysicalOne
      (by decide) (show contentHeader? _ = some (2, 3) by decide)
      (by unfold tapeLength PairEncoding.pairLength; omega)
  refine ⟨hq, by rw [h1]; decide, by rw [h2]; decide, ?_⟩
  exact hblank ⟨8 + 3 + 3, by unfold tapeLength PairEncoding.pairLength; omega⟩
    (by show 8 + 3 + 2 < 8 + 3 + 3; omega)

/-- Header `(3, 5)`: gamma width two stores `100₂` at cells `12, 13, 14`; both
payload cells are past the physical word, so both digits are virtual zeros. -/
theorem probe_secondPayload_width_two_virtual_pair (B : Nat) :
    let d := FixedGammaTargetSecondPayload.machine.run
      (FixedGammaTargetSecondPayload.deadline (8 + 3))
      (FixedGammaTargetSecondPayload.startConfig B headerValueTag headerValueVirtualZero)
    d.state = FixedGammaTargetSecondPayload.qDone ∧
      d.tape ⟨8 + 3 + 1, by unfold tapeLength PairEncoding.pairLength; omega⟩ =
        some true ∧
      d.tape ⟨8 + 3 + 2, by unfold tapeLength PairEncoding.pairLength; omega⟩ =
      ...
      d.tape ⟨8 + 3 + 3, by unfold tapeLength PairEncoding.pairLength; omega⟩ =
        some false := by
  obtain ⟨zeros, -, hconsumed, -, -, hq, -, -, h1, h2, h3⟩ :=
    secondPayload_positive_register (B := B) headerValueTag headerValueVirtualZero
      (by decide) (show contentHeader? _ = some (3, 5) by decide) (by decide)
      (by unfold tapeLength PairEncoding.pairLength; omega)
  obtain rfl : zeros = 2 := by omega
  exact ⟨hq, by rw [h1]; decide, by rw [h2]; decide, by rw [h3]; decide⟩

/-- Header `(5, 5)`: gamma width two stores `110₂` at cells `13, 14, 15`; the
first payload digit is the physical one at cell `11` and the second is the
virtual zero at the blank boundary `12`. -/
theorem probe_secondPayload_width_two_virtual_tail (B : Nat) :
    let d := FixedGammaTargetSecondPayload.machine.run
      (FixedGammaTargetSecondPayload.deadline (8 + 4))
      (FixedGammaTargetSecondPayload.startConfig B headerValueTag headerValueVirtualTailOne)
    d.state = FixedGammaTargetSecondPayload.qDone ∧
      d.tape ⟨8 + 4 + 1, by unfold tapeLength PairEncoding.pairLength; omega⟩ =
        some true ∧
      d.tape ⟨8 + 4 + 2, by unfold tapeLength PairEncoding.pairLength; omega⟩ =
      ...
      d.tape ⟨8 + 4 + 3, by unfold tapeLength PairEncoding.pairLength; omega⟩ =
        some false := by
  obtain ⟨zeros, -, hconsumed, -, -, hq, -, -, h1, h2, h3⟩ :=
    secondPayload_positive_register (B := B) headerValueTag headerValueVirtualTailOne
      (by decide) (show contentHeader? _ = some (5, 5) by decide) (by decide)
      (by unfold tapeLength PairEncoding.pairLength; omega)
  obtain rfl : zeros = 2 := by omega
  exact ⟨hq, by rw [h1]; decide, by rw [h2]; decide, by rw [h3]; decide⟩

/-- Header `(6, 5)`: gamma width two stores `111₂` at cells `14, 15, 16`; here
the second payload digit is a *physical* one at cell `12`. -/
theorem probe_secondPayload_width_two_physical_pair (B : Nat) :
    let d := FixedGammaTargetSecondPayload.machine.run
      (FixedGammaTargetSecondPayload.deadline (8 + 5))
      (FixedGammaTargetSecondPayload.startConfig B headerValueTag headerValuePhysicalPair)
    d.state = FixedGammaTargetSecondPayload.qDone ∧
      d.tape ⟨8 + 5 + 1, by unfold tapeLength PairEncoding.pairLength; omega⟩ =
        some true ∧
      d.tape ⟨8 + 5 + 2, by unfold tapeLength PairEncoding.pairLength; omega⟩ =
        some true ∧
      d.tape ⟨8 + 5 + 3, by unfold tapeLength PairEncoding.pairLength; omega⟩ =
        some true := by
  obtain ⟨zeros, -, hconsumed, -, -, hq, -, -, h1, h2, h3⟩ :=
    secondPayload_positive_register (B := B) headerValueTag headerValuePhysicalPair
      (by decide) (show contentHeader? _ = some (6, 5) by decide) (by decide)
      (by unfold tapeLength PairEncoding.pairLength; omega)
  obtain rfl : zeros = 2 := by omega
  exact ⟨hq, by rw [h1]; decide, by rw [h2]; decide, by rw [h3]; decide⟩

#print axioms Pnp4.Tests.check_header_digits
#print axioms Pnp4.Tests.check_secondPayload_positive_register
#print axioms Pnp4.Tests.check_secondPayload_width_one_register
#print axioms Pnp4.Tests.check_secondPayload_zero_width_register
#print axioms Pnp4.Tests.probe_secondPayload_zero_width
#print axioms Pnp4.Tests.probe_secondPayload_width_one
#print axioms Pnp4.Tests.probe_secondPayload_width_two_virtual_pair
#print axioms Pnp4.Tests.probe_secondPayload_width_two_virtual_tail
...
section ContentFixedGammaTargetUnaryCountdownIterationBridgeSurface

open AlgorithmsToLowerBounds
open Pnp3.Complexity.Uniform.V1
open Pnp4.Frontier.ContractExpansion

theorem check_countdown_width_eq_gammaZeros {a m n consumed zeros : Nat}
    (x : Bitstring a) (w : Bitstring m)
    (hheader : contentHeader? (Fin.append x w) = some (n, consumed))
    (hg : FixedContentGammaTerminator.gammaZeros? (Fin.append x w) = some zeros) :
    zeros = gammaZeros n ∧ 2 ^ gammaZeros n ≤ n + 1 ∧ n + 1 < 2 ^ (gammaZeros n + 1) :=
  countdown_width_eq_gammaZeros x w hheader hg

theorem check_countdown_drain_cap_iff_machine_room {a m B n consumed zeros F : Nat}
    (x : Bitstring a) (w : Bitstring m)
    (hheader : contentHeader? (Fin.append x w) = some (n, consumed))
    (hg : FixedContentGammaTerminator.gammaZeros? (Fin.append x w) = some zeros) :
    zeros = gammaZeros n ∧
      ...
      (n ≤ F ∧ gammaZeros n + 2 + F ≤ a + B → 2 * (n + 1) < 2 ^ (a + B)) :=
  countdown_drain_cap_iff_machine_room x w hheader hg

theorem check_countdown_drained_header_value {a m B n consumed F : Nat}
    (x : Bitstring a) (w : Bitstring m)
    (htag : FixedContentTagGate.tagMatches (Fin.append x w) = true)
    (hheader : contentHeader? (Fin.append x w) = some (n, consumed)) (hn : 3 ≤ n)
    ...
    ∃ zeros, zeros = gammaZeros n ∧ 2 ≤ zeros ∧ consumed = 2 * zeros + 1 ∧
      ...
      d ≤ zeros ∧ c0.state = FixedGammaTargetUnaryCountdown.qLoop ∧
        ...
        (∀ i : Fin (tapeLength (PairEncoding.pairLength a m) B),
          a + m + 3 + zeros + n ≤ i.val → e.tape i = none) :=
  countdown_drained_header_value x w htag hheader hn hcap hroom

theorem check_countdown_drained_parsed_target {threshold : Nat → Nat}
    (codec : Frontier.TreeCircuitWitnessCodec threshold) {a m B F : Nat}
    (x : Bitstring a) (w : Bitstring m)
    (htag : FixedContentTagGate.tagMatches (Fin.append x w) = true)
    {pr : Σ r : Nat,
      PrefixInput
  ...
  countdown_drained_parsed_target codec x w htag hpr hn hcap hroom

/-! The first probe below runs the *actual* G2u drain out of the landed `startConfig`, for every
budget at or above the one the room premise needs; the second runs no machine at all.  The first
derives every cell from `countdown_drained_header_value`, so it pins what that theorem gives and is
not an independent machine reduction; neither runs a parser, and neither reduces the `startConfig`
the endpoint is measured from — that configuration retags an actual G2q run, which itself retags an
actual G2p-d run, and no probe evaluates either.  Neither probe claims first arrival: `qDone`
absorbs, so the later-time conjunct is persistence and nothing more.  `qDone` is an internal control
tag of a phase whose `startConfig` retags an actual prior run, so no probe below states halting of a
composed machine or language acceptance, and the twelve marks are marks, not a value in unary. -/

/-- **The whole countdown, drained on the decoded target `12`.**  The G2p-g/G2r/G2t word once more:
its tag, width three and header `(12, 7)` are pinned where it is declared, and
`probe_first_countdown_width_three` pins the entry register as `1100₂` and the first round's single
mark.  Here the register runs all the way down.  The canonical width `gammaZeros 12 = 3` is
*recovered* from the decoded header by `countdown_width_eq_gammaZeros` rather than reduced — this
probe evaluates no `bitLength` and no `Nat.log2`.  The hypothesis `9 ≤ B` is exactly what the room
`gammaZeros 12 + 2 + 12 ≤ 8 + B` asks at the lane cap `F = 12`; the borrow length is `0`, so
`fullClock 3 0 12 = 301` steps out of the landed `startConfig` reach `qDone` on the separator blank
`20` with the four register cells `16 … 19` all `some false`, `some true` at every cell of
`[21, 33)` — twelve marks — and cell `33` blank.  Step `400` shows the endpoint persisting, which is
not a claim that `301` is the first arrival. -/
theorem probe_countdown_drained_target_twelve (B : Nat) (hB : 9 ≤ B) :
    gammaZeros 12 = 3 ∧
      FixedGammaTargetRegisterDecrement.borrow headerValueTag exhaustRegisterWidthThree 3 = 0 ∧
      FixedGammaTargetUnaryCountdownIteration.fullClock 3 0 12 = 301 ∧
      let e := FixedGammaTargetUnaryCountdown.machine.run 301
        (FixedGammaTargetUnaryCountdown.startConfig B headerValueTag exhaustRegisterWidthThree)
      e.state = FixedGammaTargetUnaryCountdown.qDone ∧ e.head.val = 20 ∧
        e.tape ⟨16, by unfold tapeLength PairEncoding.pairLength; omega⟩ = some false ∧
        e.tape ⟨17, by unfold tapeLength PairEncoding.pairLength; omega⟩ = some false ∧
        e.tape ⟨18, by unfold tapeLength PairEncoding.pairLength; omega⟩ = some false ∧
        e.tape ⟨19, by unfold tapeLength PairEncoding.pairLength; omega⟩ = some false ∧
        (∀ i : Fin (tapeLength (PairEncoding.pairLength 8 7) B), 21 ≤ i.val → i.val < 33 →
          e.tape i = some true) ∧
        e.tape ⟨33, by unfold tapeLength PairEncoding.pairLength; omega⟩ = none ∧
        (FixedGammaTargetUnaryCountdown.machine.run 400
          (FixedGammaTargetUnaryCountdown.startConfig B headerValueTag
            exhaustRegisterWidthThree)).state = FixedGammaTargetUnaryCountdown.qDone := by
  have hb : FixedGammaTargetRegisterDecrement.borrow headerValueTag
      exhaustRegisterWidthThree 3 = 0 := by decide
  have hgz : gammaZeros 12 = 3 :=
    (check_countdown_width_eq_gammaZeros headerValueTag exhaustRegisterWidthThree
      (show contentHeader? _ = some (12, 7) by decide)
      (show FixedContentGammaTerminator.gammaZeros? _ = some 3 by decide)).1.symm
  obtain ⟨zeros, -, -, hconsumed, -, -, -, -, -, -, -, -, -, -, he1, he2, -, hpers, hreg, hmark,
    hlane⟩ :=
    check_countdown_drained_header_value (B := B) (F := 12) headerValueTag
      exhaustRegisterWidthThree (by decide)
      (show contentHeader? _ = some (12, 7) by decide) (by omega) (by omega) (by omega)
  obtain rfl : zeros = 3 := by omega
  rw [hb] at he1 he2 hpers hreg hmark hlane
  rw [show FixedGammaTargetUnaryCountdownIteration.fullClock 3 0 12 = 301 from
    rfl] at he1 he2 hpers hreg hmark hlane
  have hlt : ∀ j : Nat, j ≤ 33 → j < tapeLength (PairEncoding.pairLength 8 7) B := by
    unfold tapeLength PairEncoding.pairLength
    omega
  refine ⟨hgz, hb, rfl, he1, by omega,
    hreg 0 (by omega) ⟨16, hlt 16 (by omega)⟩ (by rfl),
    hreg 1 (by omega) ⟨17, hlt 17 (by omega)⟩ (by rfl),
    hreg 2 (by omega) ⟨18, hlt 18 (by omega)⟩ (by rfl),
    hreg 3 (by omega) ⟨19, hlt 19 (by omega)⟩ (by rfl),
    fun i h1 h2 => hmark i (by omega) (by omega),
    hlane ⟨33, hlt 33 (by omega)⟩ (show 8 + 7 + 3 + 3 + 12 ≤ 33 by omega), ?_⟩
  rw [hpers 400 (by omega)]
  exact he1

/-- **The full-drain room is strictly stronger than the first round's, and no lane cap rescues
it.**  On the same word at the budget `B = 0` the G2t room `2 * (12 + 1) < 2 ^ (8 + 0)` holds, and
so does its tape form: the *first* tally cell `8 + 7 + 3 + 3 = 21` is a cell of
`tapeLength (pairLength 8 7) 0`.  The cell `21 + 12 = 33` that a twelve-mark lane's room premise
reserves is not, and correspondingly the G2u room fails for *every* lane cap the countdown admits
here, since `n ≤ F` already forces `12 ≤ F`.  So `first_countdown_header_value` applies to this
split and `countdown_drained_header_value` does not, at any `F`: the strengthening is real and not a
restatement.  Nothing below says that the twelve-mark lane *would* fit under some other premise, and
no run occurs: the last conjunct is about the room, not about an execution.  The four arithmetic
conjuncts are independent kernel facts about literals; only the width identification
`gammaZeros 12 = 3` is derived, from the decoded header. -/
theorem probe_countdown_drain_room_strictly_stronger :
    gammaZeros 12 = 3 ∧
      2 * (12 + 1) < 2 ^ (8 + 0) ∧
      8 + 7 + 3 + 3 < tapeLength (PairEncoding.pairLength 8 7) 0 ∧
      ¬ (8 + 7 + 3 + 3 + 12 < tapeLength (PairEncoding.pairLength 8 7) 0) ∧
      (∀ F : Nat, 12 ≤ F → ¬ (gammaZeros 12 + 2 + F ≤ 8 + 0)) := by
  have hgz : gammaZeros 12 = 3 :=
    (check_countdown_width_eq_gammaZeros headerValueTag exhaustRegisterWidthThree
      (show contentHeader? _ = some (12, 7) by decide)
      (show FixedContentGammaTerminator.gammaZeros? _ = some 3 by decide)).1.symm
  refine ⟨hgz, by norm_num, by unfold tapeLength PairEncoding.pairLength; omega,
    by unfold tapeLength PairEncoding.pairLength; omega, fun F hF => by omega⟩

#print axioms Pnp4.Tests.check_countdown_width_eq_gammaZeros
#print axioms Pnp4.Tests.check_countdown_drain_cap_iff_machine_room
#print axioms Pnp4.Tests.check_countdown_drained_header_value
#print axioms Pnp4.Tests.check_countdown_drained_parsed_target
#print axioms Pnp4.Tests.probe_countdown_drained_target_twelve
#print axioms Pnp4.Tests.probe_countdown_drain_room_strictly_stronger

end ContentFixedGammaTargetUnaryCountdownIterationBridgeSurface

section ContentCountdownLinearCapSurface

open AlgorithmsToLowerBounds
open Pnp3.Complexity.Uniform.V1
open Pnp4.Frontier.ContractExpansion

theorem check_contentSemanticAccepts_parsed_target_le_length (k : Nat) {N : Nat}
    (z : PrefixBitVec N)
    {pr : Σ r : Nat,
      PrefixInput
    ...
    pr.2.n ≤ N :=
  contentSemanticAccepts_parsed_target_le_length k z hpr haccept

theorem check_contentSemanticAccepts_eq_false_of_length_lt_parsed_target (k : Nat) {N : Nat}
    (z : PrefixBitVec N)
    {pr : Σ r : Nat,
      PrefixInput
        (Frontier.treeMCSPSearchProblem (thresholdPoly k)
    ...
    (hpr : contentInput? (treeCircuitWitnessCodec (thresholdPoly k)) z = some pr)
    (hlt : N < pr.2.n) :
    contentSemanticAccepts (treeCircuitWitnessCodec (thresholdPoly k)) z = false :=
  contentSemanticAccepts_eq_false_of_length_lt_parsed_target k z hpr hlt

theorem check_contentSemanticAccepts_parsed_target_le_pair_length (k : Nat) {a m : Nat}
    (x : Bitstring a) (w : Bitstring m)
    {pr : Σ r : Nat,
      PrefixInput
        (Frontier.treeMCSPSearchProblem (thresholdPoly k)
...
theorem check_countdown_drained_accepted_content (k : Nat) {a m B : Nat}
    (x : Bitstring a) (w : Bitstring m)
    (htag : FixedContentTagGate.tagMatches (Fin.append x w) = true)
    {pr : Σ r : Nat,
      PrefixInput
...
the successful parse, and `contentSemanticAccepts_eq_true_iff` turns the predicate into the Boolean
form the cap theorem reads.  So `contentSemanticAccepts_parsed_target_le_length` is not a statement
about an empty premise set, and its conclusion is exhibited on a word that exists.  This probe pins
no target value: it constructs no word itself and reads none of `z`'s cells.  It is not a
complexity, machine or language-membership claim. -/
theorem probe_linear_cap_accepted_nonvacuous (k n : Nat) :
    ∃ (z : PrefixBitVec
            (treeMCSPPrefixM (treeCircuitWitnessCodec (thresholdPoly k)) n
              + Pnp3.ComplexityInterfaces.certificateLength
                  (treeMCSPPrefixM (treeCircuitWitnessCodec (thresholdPoly k)) n) 1))
        (pr : Σ r : Nat,
          PrefixInput
            ...
            (treeMCSPPrefixM (treeCircuitWitnessCodec (thresholdPoly k)) r)),
      contentInput? (treeCircuitWitnessCodec (thresholdPoly k)) z = some pr ∧
      contentSemanticAccepts (treeCircuitWitnessCodec (thresholdPoly k)) z = true ∧
      pr.2.n ≤ treeMCSPPrefixM (treeCircuitWitnessCodec (thresholdPoly k)) n
        + Pnp3.ComplexityInterfaces.certificateLength
            (treeMCSPPrefixM (treeCircuitWitnessCodec (thresholdPoly k)) n) 1 := by
  obtain ⟨z, hz⟩ := contentAccepts_nonvacuous_treePoly k n
  have haccept : contentSemanticAccepts (treeCircuitWitnessCodec (thresholdPoly k)) z = true :=
    (contentSemanticAccepts_eq_true_iff _ z).2 hz
  obtain ⟨pr, hpr, -, -⟩ := hz
  exact ⟨z, pr, hpr, haccept,
    check_contentSemanticAccepts_parsed_target_le_length k z hpr haccept⟩

#print axioms Pnp4.Tests.check_contentSemanticAccepts_parsed_target_le_length
...
section ContentFixedGammaTargetLoopDecrementCountdownBridgeSurface

open AlgorithmsToLowerBounds
open Pnp3.Complexity.Uniform.V1
open Pnp4.Frontier.ContractExpansion

theorem check_loop_decrement_countdown_drained_accepted_content_at_polyClock (k : Nat) {a m : Nat}
    (x : Bitstring a) (w : Bitstring m)
    {pr : Σ r : Nat,
      PrefixInput
        (Frontier.treeMCSPSearchProblem (thresholdPoly k)
    ...
    pr.2.n ≤ a + m ∧ 11 ≤ a + m ∧
      ...
      ∃ zeros, zeros = gammaZeros pr.2.n ∧ 2 ≤ zeros ∧ pr.2.n = pr.1 ∧
        ...
        e.state = M.accept ∧ e.head.val = a + m + 2 + zeros ∧
          e.tape = FixedGammaTargetUnaryCountdown.loopTape B x w zeros 0 pr.2.n ∧
          M.run B c = e ∧ (∀ t, C ≤ t → M.run t c = e) :=
  loop_decrement_countdown_drained_accepted_content_at_polyClock k x w hpr haccept hn

/-- **The G2y endpoint's three premises are jointly inhabited, at a pinned target, and the newly
executed handoff H16 is read back.**  The word and the three premises are those of
`probe_countdown_polyClock_accepted_target_three` — GATE-0's zero-prefix query for the all-false
table on three variables plus its certificate, parsed at target `3`; nothing new is constructed.
On it the composed 40-state machine has a switch time `T ≤ B` before which it is in neither verdict
and at which it is G2x's landed `startConfig` re-embedded, and after exactly `B` steps it is in the
composed accept.  One word per exponent: no rejected word, no overshooting word, no tape cell.
Reaching the composed accept here is neither halting on a raw input, nor language acceptance, nor
an `NP`-membership claim. -/
theorem probe_loop_decrement_countdown_polyClock_accepted_target_three (k : Nat) :
    ∃ (w : Pnp3.ComplexityInterfaces.Bitstring
            (Pnp3.ComplexityInterfaces.certificateLength
              (treeMCSPPrefixM (treeCircuitWitnessCodec (thresholdPoly k)) 3) 1))
        (pr : Σ r : Nat,
      ...
      let B := polyClock 3 (PairEncoding.pairLength
        ...
        (Pnp3.ComplexityInterfaces.certificateLength
          (treeMCSPPrefixM (treeCircuitWitnessCodec (thresholdPoly k)) 3) 1))
      let M := FixedGammaTargetLoopDecrementCountdown.machine
      let c := FixedGammaTargetLoopDecrementCountdown.startConfig B y w
      contentInput? (treeCircuitWitnessCodec (thresholdPoly k)) (Fin.append y w) = some pr ∧
        contentSemanticAccepts (treeCircuitWitnessCodec (thresholdPoly k))
          (Fin.append y w) = true ∧
        3 ≤ pr.2.n ∧ pr.2.n = 3 ∧
        ∃ T, T ≤ B ∧
          ...
          M.run T c =
            FixedGammaTargetPayloadRound.machine.seqEmbedRight
              FixedGammaTargetDecrementCountdown.machine
              (FixedGammaTargetDecrementCountdown.startConfig B y w) ∧
          (M.run B c).state = M.accept := by
  obtain ⟨w, pr, hpr, haccept, hn, hthree, -, -⟩ :=
    probe_countdown_polyClock_accepted_target_three k
  obtain ⟨-, -, -, zeros, -, -, -, -, -, hCT, hCB, hfirst, hT, he1, -, -, hB, -⟩ :=
    loop_decrement_countdown_drained_accepted_content_at_polyClock k
      (zeroPrefixQueryValue (treeCircuitWitnessCodec (thresholdPoly k)) 3 (fun _ => false)) w
      hpr haccept hn
  refine ⟨w, pr, hpr, haccept, hn, hthree, _, by omega, hfirst, hT, ?_⟩
  rw [hB]
...
#print axioms Pnp4.Tests.check_loop_decrement_countdown_drained_accepted_content_at_polyClock
#print axioms Pnp4.Tests.probe_loop_decrement_countdown_polyClock_accepted_target_three

end ContentFixedGammaTargetLoopDecrementCountdownBridgeSurface

section ContentFixedGammaTargetMarkersLoopDecrementCountdownBridgeSurface

open AlgorithmsToLowerBounds
open Pnp3.Complexity.Uniform.V1
open Pnp4.Frontier.ContractExpansion

theorem check_markers_loop_decrement_countdown_drained_accepted_content_at_polyClock (k : Nat)
    ...
    pr.2.n ≤ a + m ∧ 11 ≤ a + m ∧
      ...
      ∃ zeros, zeros = gammaZeros pr.2.n ∧ 2 ≤ zeros ∧ pr.2.n = pr.1 ∧
        ...
        e.state = M.accept ∧ e.head.val = a + m + 2 + zeros ∧
          e.tape = FixedGammaTargetUnaryCountdown.loopTape B x w zeros 0 pr.2.n ∧
          M.run B c = e ∧ (∀ t, C ≤ t → M.run t c = e) :=
  markers_loop_decrement_countdown_drained_accepted_content_at_polyClock k x w hpr haccept hn

/-- **The G2z endpoint's three premises are jointly inhabited, at a pinned target, and the newly
executed handoff H15 is read back at a pinned time.**  The word and the three premises are those of
`probe_countdown_polyClock_accepted_target_three` — GATE-0's zero-prefix query for the all-false
table on three variables plus its certificate, parsed at target `3`; nothing new is constructed.
The parsed target `3` fixes the canonical width `gammaZeros 3 = 2`, hence the marker preamble's
first arrival `exactClock 2 = 9`: on this word the composed 54-state machine is in neither verdict
before step `9`, at exactly step `9` it *is* G2y's landed `startConfig` re-embedded, and after
exactly `B` steps it is in the composed accept.  The switch time is the literal `9`, not an
unconstrained existential.  One word per exponent: no rejected word, no overshooting word, no tape
cell.  Reaching the composed accept here is neither halting on a raw input, nor language acceptance,
nor an `NP`-membership claim. -/
theorem probe_markers_loop_decrement_countdown_polyClock_accepted_target_three (k : Nat) :
    ∃ (w : Pnp3.ComplexityInterfaces.Bitstring
            (Pnp3.ComplexityInterfaces.certificateLength
              (treeMCSPPrefixM (treeCircuitWitnessCodec (thresholdPoly k)) 3) 1))
        (pr : Σ r : Nat,
      ...
      let B := polyClock 3 (PairEncoding.pairLength
        ...
        (Pnp3.ComplexityInterfaces.certificateLength
          (treeMCSPPrefixM (treeCircuitWitnessCodec (thresholdPoly k)) 3) 1))
      let M := FixedGammaTargetMarkersLoopDecrementCountdown.machine
      let c := FixedGammaTargetMarkersLoopDecrementCountdown.startConfig B y w
      contentInput? (treeCircuitWitnessCodec (thresholdPoly k)) (Fin.append y w) = some pr ∧
        contentSemanticAccepts (treeCircuitWitnessCodec (thresholdPoly k))
          (Fin.append y w) = true ∧
        3 ≤ pr.2.n ∧ pr.2.n = 3 ∧ gammaZeros pr.2.n = 2 ∧ 9 ≤ B ∧
        (∀ t, t < 9 → (M.run t c).state ≠ M.accept ∧ (M.run t c).state ≠ M.reject) ∧
        M.run 9 c =
          FixedGammaTargetPayloadLoopFoundation.machine.seqEmbedRight
            FixedGammaTargetLoopDecrementCountdown.machine
            (FixedGammaTargetLoopDecrementCountdown.startConfig B y w) ∧
        (M.run B c).state = M.accept := by
  obtain ⟨w, pr, hpr, haccept, hn, hthree, hwidth, -⟩ :=
    probe_countdown_polyClock_accepted_target_three k
  obtain ⟨-, -, -, zeros, hzg, -, -, -, -, hCS, hCB, hfirst, hS, he1, -, -, hB, -⟩ :=
    markers_loop_decrement_countdown_drained_accepted_content_at_polyClock k
      (zeroPrefixQueryValue (treeCircuitWitnessCodec (thresholdPoly k)) 3 (fun _ => false)) w
      hpr haccept hn
  have hz2 : zeros = 2 := hzg.trans hwidth
  subst hz2
  rw [show FixedGammaTargetPayloadLoopFoundation.exactClock 2 = 9 from rfl] at hfirst hS hCS
  refine ⟨w, pr, hpr, haccept, hn, hthree, hwidth, by omega, hfirst, hS, ?_⟩
  rw [hB]
  exact he1

#print axioms Pnp4.Tests.check_markers_loop_decrement_countdown_drained_accepted_content_at_polyClock
#print axioms Pnp4.Tests.probe_markers_loop_decrement_countdown_polyClock_accepted_target_three

end ContentFixedGammaTargetMarkersLoopDecrementCountdownBridgeSurface

section ContentFixedGammaTargetSecondPayloadMarkersLoopDecrementCountdownBridgeSurface

open AlgorithmsToLowerBounds
open Pnp3.Complexity.Uniform.V1
open Pnp4.Frontier.ContractExpansion

theorem check_second_payload_markers_loop_decrement_countdown_drained_accepted_content_at_polyClock
    ...
    pr.2.n ≤ a + m ∧ 11 ≤ a + m ∧
      ...
      ∃ zeros, zeros = gammaZeros pr.2.n ∧ 2 ≤ zeros ∧ pr.2.n = pr.1 ∧
        ...
        e.state = M.accept ∧ e.head.val = a + m + 2 + zeros ∧
          e.tape = FixedGammaTargetUnaryCountdown.loopTape B x w zeros 0 pr.2.n ∧
          M.run B c = e ∧ (∀ t, C ≤ t → M.run t c = e) :=
  second_payload_markers_loop_decrement_countdown_drained_accepted_content_at_polyClock k x w
    hpr haccept hn

/-- **The G3a endpoint's three premises are jointly inhabited, at a pinned target, and the newly
executed handoff H14 is read back at a pinned time.**  The word and the three premises are those of
`probe_countdown_polyClock_accepted_target_three` — GATE-0's zero-prefix query for the all-false
table on three variables plus its certificate, parsed at target `3`; nothing new is constructed.
The parsed target `3` fixes the canonical width `gammaZeros 3 = 2`, hence the positive-width branch
and with it G2p-c's first arrival.  Unlike G2z's preamble arrival, that time is **length-dependent**:
it is `2 * N - 7` for the derived `N = a + m`, not a numeral, so the probe pins it by that closed
formula together with the exported `11 ≤ N`.  On this word the composed 68-state machine is in
neither verdict before `2 * N - 7`, at exactly `2 * N - 7` it *is* G2z's landed `startConfig`
re-embedded, and after exactly `B` steps it is in the composed accept.  One word per exponent: no
rejected word, no overshooting word, no tape cell, and neither degenerate width.  Reaching the
composed accept here is neither halting on a raw input, nor language acceptance, nor an
`NP`-membership claim. -/
theorem probe_second_payload_markers_loop_decrement_countdown_polyClock_accepted_target_three
    (k : Nat) :
    ∃ (w : Pnp3.ComplexityInterfaces.Bitstring
            (Pnp3.ComplexityInterfaces.certificateLength
              (treeMCSPPrefixM (treeCircuitWitnessCodec (thresholdPoly k)) 3) 1))
      ...
      let B := polyClock 3 (PairEncoding.pairLength
        ...
        (Pnp3.ComplexityInterfaces.certificateLength
          (treeMCSPPrefixM (treeCircuitWitnessCodec (thresholdPoly k)) 3) 1))
      let M := FixedGammaTargetSecondPayloadMarkersLoopDecrementCountdown.machine
      let c := FixedGammaTargetSecondPayloadMarkersLoopDecrementCountdown.startConfig B y w
      contentInput? (treeCircuitWitnessCodec (thresholdPoly k)) (Fin.append y w) = some pr ∧
        contentSemanticAccepts (treeCircuitWitnessCodec (thresholdPoly k))
          (Fin.append y w) = true ∧
        3 ≤ pr.2.n ∧ pr.2.n = 3 ∧ gammaZeros pr.2.n = 2 ∧ 11 ≤ N ∧ 2 * N - 7 ≤ B ∧
        (∀ t, t < 2 * N - 7 →
          (M.run t c).state ≠ M.accept ∧ (M.run t c).state ≠ M.reject) ∧
        M.run (2 * N - 7) c =
          FixedGammaTargetSecondPayload.machine.seqEmbedRight
            FixedGammaTargetMarkersLoopDecrementCountdown.machine
            (FixedGammaTargetMarkersLoopDecrementCountdown.startConfig B y w) ∧
        (M.run B c).state = M.accept := by
  obtain ⟨w, pr, hpr, haccept, hn, hthree, hwidth, -⟩ :=
    probe_countdown_polyClock_accepted_target_three k
  obtain ⟨-, hN, -, zeros, hzg, -, -, -, -, hSval, hCS, hCB, hfirst, hS, he1, -, -, hB, -⟩ :=
    second_payload_markers_loop_decrement_countdown_drained_accepted_content_at_polyClock k
      (zeroPrefixQueryValue (treeCircuitWitnessCodec (thresholdPoly k)) 3 (fun _ => false)) w
      hpr haccept hn
  rw [hSval] at hfirst hS hCS
  refine ⟨w, pr, hpr, haccept, hn, hthree, hwidth, hN, by omega, hfirst, hS, ?_⟩
  rw [hB]
  exact he1

#print axioms
  Pnp4.Tests.check_second_payload_markers_loop_decrement_countdown_drained_accepted_content_at_polyClock
#print axioms
  Pnp4.Tests.probe_second_payload_markers_loop_decrement_countdown_polyClock_accepted_target_three

end ContentFixedGammaTargetSecondPayloadMarkersLoopDecrementCountdownBridgeSurface

section ContentFixedGammaTargetFirstPayloadSecondPayloadMarkersLoopDecrementCountdownBridgeSurface

open AlgorithmsToLowerBounds
open Pnp3.Complexity.Uniform.V1
open Pnp4.Frontier.ContractExpansion

theorem check_first_payload_second_payload_markers_loop_decrement_countdown_drained_accepted_content_at_polyClock
    (k : Nat) {a m : Nat} (x : Bitstring a) (w : Bitstring m)
    {pr : Σ r : Nat,
      PrefixInput
        (Frontier.treeMCSPSearchProblem (thresholdPoly k)
    ...
    (hn : 3 ≤ pr.2.n) :
    let B := polyClock 3 (PairEncoding.pairLength a m)
    let M := FixedGammaTargetFirstPayloadSecondPayloadMarkersLoopDecrementCountdown.machine
    let c := FixedGammaTargetFirstPayloadSecondPayloadMarkersLoopDecrementCountdown.startConfig
      B x w
    pr.2.n ≤ a + m ∧ 11 ≤ a + m ∧
      FixedContentTagGate.tagMatches (Fin.append x w) = true ∧
      ∃ zeros, zeros = gammaZeros pr.2.n ∧ 2 ≤ zeros ∧ pr.2.n = pr.1 ∧
        ...
        FixedContentGammaTerminator.gammaZeros? (Fin.append x w) = some zeros ∧
        let d := FixedGammaTargetRegisterDecrement.borrow x w zeros
        let S := FixedGammaTargetFirstPayload.exactClock (a + m) zeros
        let C := FixedGammaTargetFirstPayloadSecondPayloadMarkersLoopDecrementCountdown.firstChainClock
          (a + m) zeros d pr.2.n
        S = 2 * (a + m) + zeros - 6 ∧
        C = S + FixedGammaTargetSecondPayloadMarkersLoopDecrementCountdown.secondChainClock
          (a + m) zeros d pr.2.n ∧
          C ≤ B ∧
        (∀ t, t < S → (M.run t c).state ≠ M.accept ∧ (M.run t c).state ≠ M.reject) ∧
        M.run S c =
...
/-- **The G3c endpoint's three premises are jointly inhabited, at a pinned target, and the newly
executed handoff H13 is read back at a pinned time.**  The word and the three premises are those of
`probe_countdown_polyClock_accepted_target_three` — GATE-0's zero-prefix query for the all-false
table on three variables plus its certificate, parsed at target `3`; nothing new is constructed.
The parsed target `3` fixes the canonical width `gammaZeros 3 = 2`, hence a positive width and with
it G2p-b's first arrival.  Like G2p-c's, that time is **length-dependent**, and unlike it, it also
moves with the width: it is `2 * N + zeros - 6` for the derived `N = a + m`, here `2 * N + 2 - 6`,
which the probe also records as `2 * N - 4`; both are pinned by closed formula together with the
exported `11 ≤ N`, not by a numeral.  On this word the composed 86-state machine is in neither
verdict before that time, at exactly that time it *is* G3a's landed `startConfig` re-embedded, and
after exactly `B` steps it is in the composed accept.  One word per exponent: no rejected word, no
overshooting word, no tape cell, and not the degenerate width.  Reaching the composed accept here is
neither halting on a raw input, nor language acceptance, nor an `NP`-membership claim. -/
theorem probe_first_payload_second_payload_markers_loop_decrement_countdown_polyClock_accepted_target_three
    (k : Nat) :
    ∃ (w : Pnp3.ComplexityInterfaces.Bitstring
            (Pnp3.ComplexityInterfaces.certificateLength
              (treeMCSPPrefixM (treeCircuitWitnessCodec (thresholdPoly k)) 3) 1))
      ...
      contentInput? (treeCircuitWitnessCodec (thresholdPoly k)) (Fin.append y w) = some pr ∧
        ...
        2 * N + 2 - 6 = 2 * N - 4 ∧ 2 * N + 2 - 6 ≤ B ∧
        (∀ t, t < 2 * N + 2 - 6 →
          (M.run t c).state ≠ M.accept ∧ (M.run t c).state ≠ M.reject) ∧
        M.run (2 * N + 2 - 6) c =
          FixedGammaTargetFirstPayload.machine.seqEmbedRight
            FixedGammaTargetSecondPayloadMarkersLoopDecrementCountdown.machine
            (FixedGammaTargetSecondPayloadMarkersLoopDecrementCountdown.startConfig B y w) ∧
        (M.run B c).state = M.accept := by
  obtain ⟨w, pr, hpr, haccept, hn, hthree, hwidth, -⟩ :=
    probe_countdown_polyClock_accepted_target_three k
  obtain ⟨-, hN, -, zeros, hzg, -, -, -, -, hSval, hCS, hCB, hfirst, hS, he1, -, -, hB, -⟩ :=
  ...
  rw [hSval] at hfirst hS hCS
  refine ⟨w, pr, hpr, haccept, hn, hthree, hwidth, hN, by omega, by omega, hfirst, hS, ?_⟩
  rw [hB]
  exact he1

#print axioms
  Pnp4.Tests.check_first_payload_second_payload_markers_loop_decrement_countdown_drained_accepted_content_at_polyClock
#print axioms
