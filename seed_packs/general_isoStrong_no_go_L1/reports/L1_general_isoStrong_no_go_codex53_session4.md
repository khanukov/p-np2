...
The final L1 route-level assembly is completed. For arbitrary
`F : GapSliceFamilyEventually` and arbitrary per-slice
`hInDag : ∀ n β, InPpolyDAG (gapPartialMCSP_Language (F.paramsOf n β))`,
we now derive

`¬ IsoStrongFamilyEventually F hInDag`.

So the general iso-strong conclusion route is formally refuted at this probe layer.

## 4. Remaining blockers

None at L1 session-4 scope for the stated target theorem.
